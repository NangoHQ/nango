...
if [ "$PACKAGE_NAME" == "jobs" ] || [ "$PACKAGE_NAME" == "runner" ]; then
    ...
    if [ "$ENV" == "enterprise" ]; then
        npm run build:enterprise && docker buildx build --platform linux/amd64 -f packages/$PACKAGE_NAME/Dockerfile -t nangohq/$package:enterprise . --no-cache --output type=registry
    fi
