...
const apiCatalogGroup = integrationsTab.groups?.find((group) => group.group === '700+ APIs & Integrations');

if (!apiOverviewGroup || !apiCatalogGroup) {
    throw new Error('Expected Overview and 700+ APIs & Integrations groups in docs/docs.json');
}
...
async function buildCatalog(routes: string[]): Promise<CatalogEntry[]> {
    ...
    for (const slug of slugs) {
        ...
        const setupRoute = provider?.setup_guide_url ? urlToDocsRoute(provider.setup_guide_url) : routeForExistingSetupPage(routeWithFile);

        entries.set(slug, {
            slug,
            title: provider?.display_name ?? meta.title,
