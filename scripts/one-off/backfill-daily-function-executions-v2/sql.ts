...
export function verificationQueries(database: string, rawFloor: string): string {
    return `SELECT day, sum(value), sum(duration_ms), sum(duration_seconds), sum(compute_gbms), sum(compute_gbs) FROM ${database}.daily_function_executions_v2 GROUP BY day ORDER BY day;
