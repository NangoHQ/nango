...
function buildAgentPrompt(params: { providerNames: string[]; baseRef: string; providersYamlPath: string; providersScopesPath: string }): string {
    ...
    return [
        ...
        '  A1) Read providers.yaml to get the authorization_url for the provider (e.g. https://auth.example.com/oauth/authorize).',
        ...
        '       - Only attempt the discovery request if the derived base is a bare origin (scheme + host + optional port, no path). If it has a path, skip it rather than making a request that will certainly 404.',
        ...
        '7) Exclude scopes that are not passed via the OAuth authorization URL scope parameter.',
