...
In both cases, you need to transfer data from the external system to your app, either on a schedule or in real-time. In Nango, this is achieved with a [sync](/understand/concepts/syncs). For a 2-way sync, you can write back to the external system using [actions](/understand/concepts/actions).
...
For small datasets (e.g., a list of Slack users for organizations with less than 100 employees), you can instruct Nango to periodically poll for the entire dataset. This method is known as a ["full refresh" sync](/understand/concepts/syncs#full-refresh-syncs).
...
The preferred method for syncing larger datasets is to fetch only the incremental changes from the external API. This method is known as an ["incremental" sync](https://docs.nango.dev/understand/concepts/syncs#incremental-syncs). 

Nango automatically stores the timestamp of the last sync execution start. You can use this timestamp to instruct the external API to send only the changes since that date. That way, you only receive and persist the modified records in the Nango cache.
...
Here is an example of a sync script that updates the list of Salesforce contact incrementally on each execution:
...
Nango integration scripts, which manage data syncing between external systems and Nango, run on customer-specific VMs with [fixed resources](/understand/concepts/scripts#performance-considerations). Consequently, integration scripts can lead to sync failures (e.g., VM crash) when memory resources are overused. 
...
You can monitor the memory consumption of your script executions in the Activity tab within the Nango UI. 
