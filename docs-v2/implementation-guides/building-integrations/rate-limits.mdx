...
- Nango [syncs](/guides/use-cases/syncs) can run for up to 24 hours, allowing retries to occur within this window (note that action and webhook scripts have shorter lifespans, see [Resource Limits](/guides/platform/resource-limits))
- It works for APIs without requiring any API-specific configurations

Find details about HTTP request retries in the [reference](/reference/scripts#http-request-retries).
