...
Nango publishes reference implementations for the most popular APIs & use cases. Reference implementations are a starting point for your custom integrations to reuse and extend.
...
You can either copy an entire integration folder (e.g. `github`), or copy individual sync & action files. If you copy individual files, make sure you also copy imported functions that may be in other files.
...
Edit the sync & action functions as required for your integration.
...
These two guides explain the SDK available in custom sync & action functions. They also explain how to test your functions locally, and how to deploy them.
