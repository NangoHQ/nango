...
Sync scripts expose the timestamp of the last sync execution start under `nango.lastSyncDate`. You can use this timestamp to instruct the external API to send only the changes since that date. This way, you only receive and persist the modified records in the Nango cache.
...
You can also use [customer-specific config](/guides/platform/customer-configuration) to implement customer-specific filters in your sync scripts.
