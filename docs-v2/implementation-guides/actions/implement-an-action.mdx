...
Action files should be within a `actions` folder, which is nested under the integration's folder.
...
Reference implementations are a starting point. You will likely need to [customize them]() or [create your own custom action](#how-to-build-an-action).
...
Follow the [implement the MCP server]() guide to set this up.