...
<Note>
Make sure you are not running `npx nango start` somewhere, if so kill the tab. It is redundant with the following command.
</Note>
...
<Tip>
	...
	`npx nango dev` runs Nango (like `npx nango start`) but also automatically watches for changes in your files and recompiles as needed. Compilation errors are printed to the terminal.
...
To do not need to specify the `providerConfigKey` and `connectionId` fields in the call to the Proxy. They are automatically injected.
</Tip>
