...
<Tip>
    Before using sync you need to setup auth by following the [Authorize APIs guide](/guides/oauth).
</Tip>

## When to use _syncs_? 

_Sync_ enable to continuously sync data between your app and external APIs. This is useful when you need to: 
...
_Syncs_ handles the critical infrastructure & logic necessary to continuously & reliably sync data:
- API requests & data transformation
- Unification of schemas across external APIs
- Logging & monitoring
- Rate-limit handling
- Automated retries
- _Syncs_ API
...
import type { AsanaTask } from '<path-to-nango-integrations>/models'

const nango = new Nango({ secretKey: '<your-dev-secret-key>' });

const result = await nango.getRecords<AsanaTask>({
...
You can store field mapping data that is specific to a _connection_ in Nango with [custom connection metadata](/guides/oauth#storing-custom-metadata-per-connection).
...
_Syncs_ have the capability of detecting deletions. Deleted records are preserved in Nango but marked as deleted in the record metadata (i.e. _soft deletes_). You can include the metadata in fetched record using the [API](/api-reference/sync/records-get)'s `include_nango_metadata` query param or the [SDK](/sdks/node#read-synced-data)'s `includeNangoMetadata` param.

There are two different ways to detect deletes based on wether a _sync_ script works incrementally (i.e. each _sync_ job only fetches the changes since the last _sync_ job from the external API) or with full refresh (i.e. each _sync_ job fetches all the data from the external API).

**Incremental _syncs_**

For incremental _syncs_, you can use the `batchDelete` method in your _sync_ scripts to flag records as deleted:
```ts
await nango.batchDelete<User>(users, 'User');
```

The objects marked as deleted (here `users`) only need to contain the unique `id` field. 

**Full refresh _syncs_**

For full refresh _syncs_, you simply need to add a `track_deletes: true` field for your _sync_ in your `nango.yaml` configuration. With this configuration, when an previously returned by the _sync_ script is no longer returned, it will be marked as deleted.
