...
- We recommend enabling [Nango's revoked token webhooks](http://localhost:3000/guides/webhooks/webhooks-from-nango#auth-webhooks) for real-time notifications.

Revoked refresh tokens are a normal part of integrations. Ensure your app provides a good re-auth experience for users.

APIs may revoke tokens for unpredictable reasons. Only investigate if you see 10%+ of your connections for a provider revoked at once. Achieving a 0% revocation rate is not realistic.
