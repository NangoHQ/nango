...
You can access the metadata from any [Function]() in Nango, so your integration can read mappings, filters, and flags at runtime.

To implement per-customer config in your Nango integrations, follow the step‑by‑step guide: [Implement per-customer configuration](/implementation-guides/building-integrations/best-practices/customer-configuration).
