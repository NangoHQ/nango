...
An integration is created on Nango's dashboard and powered by a set of scripts. A script combines it's configuration and logic.

Scripts are developed locally using the Nango CLI and deployed independently to your Nango workspace. Once deployed, each integration creates a Nango API endpoint to:
...
1. **Build and define scripts**: Use the CLI to develop your TypeScript scripts. A script is self-contained and defines it's own configuration and logic:
   - Input/output models.
   - Schedules and operational modes (e.g., incremental or full refresh for syncs).
2. **Test locally**: Validate scripts using the CLI before deploying them.
3. **Deploy to Nango**: Push scripts to Nango's infrastructure for execution.
