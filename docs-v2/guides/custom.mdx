...
In the folder where you want your integrations folder (e.g. root of your project), which
will create './nango-integrations' with initial config, run:
```bash
nango init
```
...
which will continuously watch integration files for changes:
```bash
nango dev
```
