...
Integration scripts, whether provided by Nango as [templates](/understand/concepts/templates) or developed by customers ([step-by-step guide](/guides/customize/create-a-custom-integration)), are stored in blob storage. This provides a scalable, secure storage solution that ensures integration scripts are readily accessible for execution by the runner services.
