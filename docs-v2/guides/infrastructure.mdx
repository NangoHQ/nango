...
Integration scripts, whether provided by Nango as [templates](/guides/pre-built-integrations/overview) or developed by customers ([step-by-step guide](/guides/custom-integration-builder/overview)), are stored in blob storage. This provides a scalable, secure storage solution that ensures integration scripts are readily accessible for execution by the runner services.
