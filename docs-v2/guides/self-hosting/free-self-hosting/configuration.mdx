...
<Note>
    For external databases using poolers, Nango does not play well with `pool_mode=transaction`
     which some providers (e.g. Neon and DigitalOcean) use in their poolers. Use a direct connection or change the pooler mode.
</Note>
