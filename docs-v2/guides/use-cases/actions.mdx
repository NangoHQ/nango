...
description: 'Create new API endpoints on the external API'
---

Actions let you create new API endpoints on the external API.

You can invoke them from your backend, or expose them to LLMs with our built-in [MCP Server](/guides/use-cases/nango-mcp-server).

## When should you use actions?

- You want to create a unified interface across different APIs
- You want to write a lot of data asynchronously to the external API
- You want to abstract something that takes several external API requests into a single interface
- You want strict, end-to-end type safety for interactions with the external API

Common examples:
- Creating a unified interface to e.g. `create-contact` across several APIs (e.g. Salesforce, HubSpot, Attio, etc.)
- Creating hundreds of invoices async, as fast as rate-limits allow, in the external API (e.g. after monthly billing run)
- A simpler interface to create an opportunity, and linking it to a contact and a company, which takes 3 API requests

You can think of actions as "virtual endpoints" on the external API, that you get to define.
...
- Actions run in Nango's infrastructure, powered by [Functions](/guides/platform/functions)
    ...
    - All platform features, like data validation, per-customer config, retries, rate-limit handling, pagination helpers, are available for actions
- Actions can be used to read or write data on the external API
- Actions can call other actions
- Actions run synchronously by default, but can also run async (e.g. for batch writes)
...
- Actions can also be exposed to LLMs with our built-in [MCP Server](/guides/use-cases/nango-mcp-server)
...
To implement an async action, follow our [async acion implementation guide](/implementation-guides/actions/async-actions).
