...
### Trigger syncs
Triggers an additional, one-off sync run for all active sync scripts on the specified _connection_.

```ts
await nango.triggerSync('<INTEGRATION-ID>', '<CONNECTION-ID>');
```

You can optionally trigger for certain specified sync(s) for a _connection_:
...
```ts
await nango.pauseSync('<INTEGRATION-ID>', '<CONNECTION-ID>', ['github-sync', 'hubspot-sync']);
await nango.startSync('<INTEGRATION-ID>', '<CONNECTION-ID>', ['github-sync', 'hubspot-sync']);
```
