...
On the Dashboard, go to the [Add New Integration](http://localhost:3003/integration/create) page. 

Use the value `github` for both the `Integration Provider` dropdown and the `Integration Unique Key` field.

To get the `Client ID` and `Client Secret` you will need to register your app with Github [here](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/creating-an-oauth-app). Github will ask you to submit the `Callback URL` shown at the bottom of the [Add New Integration](http://localhost:3003/integration/create) page.
...
- Retrieve your access token from the [Connections page](http://localhost:3003/connections). 
...
Stop the Docker containers that your started in Step 1 by killing the console tab or pressing `Ctrl` + `c`.
...
```bash
npx nango sync:run -p github -s github-issue-example -c test-connection-id
```
