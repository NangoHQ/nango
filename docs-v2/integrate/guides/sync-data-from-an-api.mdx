...
```json
{
    ...
    "queryTimeStamp": "<timestamp>" // DEPRECATED: Null for initial sync runs.
}
```
...
curl -G https://api.nango.dev/records \
  ...
  --data-urlencode 'delta=<updatedAfter-from-webhook>' \  # Time cursor to only fetch the updated data.
