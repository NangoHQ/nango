...
```json
{
    ...
    "updatedAfter": "<timestamp>", // Null for initial sync runs.
    "queryTimeStamp": "<timestamp>" // DEPRECATED: (use updatedAfter) null for initial sync runs.
}
```
...
Upon receiving a webhook notification, the time cursor `updatedAfter` from the webhook body is used to fetch only the updated data.
...
curl -G https://api.nango.dev/records \
  ...
  --data-urlencode 'updated_after=<updatedAfter-from-webhook>' \  # Time cursor to only fetch the updated data.
...
const result = await nango.listRecords({
    ...
    updatedAfter: '<updatedAfter-from-webhook>', // Time cursor to only fetch the updated data.
