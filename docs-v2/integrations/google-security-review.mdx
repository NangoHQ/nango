...
If you have never done a securicty audit, you may need to create the necessary documentation and patch vulnerabilities. Budget 10-20 days.
...
If you are pressed for time and want more tips to speed things up, [reach out](https://nango.dev/slack). We are happy to help 1:1.

<Info>
    If you use restricted scopes, the CASA asseement and app verification need to be [renewed annually](https://support.google.com/cloud/answer/13463816?hl=en&ref_topic=13460882&sjid=6878633552635075588-EU). Unless you had major changes in your functionality, the renewal will be much easier.
</Info>
...
2. Prepare for the App verification (+ CASA assessment)
    ...
    - Run a first security scan on yor app to fix low-hanging fruits _(CASA assessment only)_
...
- Users see a scarry "this app is unverified" step during the OAuth flow
...
- Kepp the restricted scopes count minimal and user-scoped.
...
* For each scope you request, prepare a [detailed justification](https://support.google.com/cloud/answer/13464321?hl=en&ref_topic=13460882&sjid=4120114855682251882-EU#:~:text=Request%20narrowest%20scopes) which includes:
    ...
    * This and the demo video are most frequent source of back & forths.
...
A validations is a single scan and output of issues. If you do a pre-scan and fix issues first, you’ll probably only need the two validations to fix esoteric issues. Try to avoid scanning again without a fix or an accepted omission for everything picked up in the first scan.

Many Nango customers have used them. They are ok, cheap, and have mostly automated the process. This makes them not the best at answering emails and dealing with human interactions. We heard stories where they drop the ball on emails from Google which has caused days of delay. We recommend following up with them daily and reminding them of all communication that requires their attention.
...
You may get emails from Google asking for more details. We recommend answering quickly, but dilligently. Google answers within 24-48h, the fewer loops you need, the faster your app will get verified.
