...
<Tabs>
  ...
  **Important:** Salesforce has transitioned from **Connected Apps** to **External Client Apps**. Both Salesforce and Nango recommend using **External Client Apps** for new integrations. Here's are a [few points](https://help.salesforce.com/s/articleView?id=xcloud.connected_apps_and_external_client_apps_features.htm&type=5) you need to know between the two:
  ...
  Choose the appropriate option below based on your needs.

  ## Option A: Connected App
  ...
  **Note:** Connected Apps are disabled by default in new Salesforce organizations. To enable them:
  ...
  ## Option B: External Client App - Internal Use (Local Distribution)
