...
| Topic | Links | 
| - | - | 
| General | [ClickUp API Documentation](https://developer.clickup.com/docs) |
| | [ClickUp API Reference](https://developer.clickup.com/reference) |
| OAuth | [Authentication Documentation](https://developer.clickup.com/docs/authentication) |
...
- The access token currently does not expire. However, ClickUp notes that this is subject to change in the future.

- ClickUp uses the [authorization code](https://oauth.net/2/grant-types/authorization-code/) grant type for their OAuth flow.
...
- Non-SSL redirect URIs may not be supported in the future, so always use HTTPS for your redirect URI.

- You can add a `state` parameter to the authorization URL for additional security: `https://app.clickup.com/api?client_id={client_id}&redirect_uri={redirect_uri}&state={state}`
