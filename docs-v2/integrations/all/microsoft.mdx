...
<Steps>
  ...
  <Step title="Configure app visibility (optional)">
    ...
    1. Navigate to **Identity** > **Applications** > **Enterprise applications** and select your app.
...
Microsoft uses OAuth 2.0 for authentication. You can configure your connection in Nango using the standard authorization flow:

```js
nango.auth('microsoft', '<CONNECTION-ID>');
```
...
-   Microsoft's OAuth implementation uses the `prompt=consent` parameter by default to ensure users explicitly consent to the requested permissions.
-   Access tokens for Microsoft Graph typically expire after 1 hour. Always use refresh tokens (via the `offline_access` scope) for long-term access.
-   Microsoft's OAuth implementation supports PKCE (Proof Key for Code Exchange), but Nango has disabled it for this provider as shown in the providers.yaml configuration.
