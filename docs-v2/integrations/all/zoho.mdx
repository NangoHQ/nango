...
- Access tokens are valid for 1 hour by default. Use the refresh token to obtain a new access token when needed.

- When requesting an access token, include `access_type=offline` in your authorization request to receive a refresh token.

- Refresh tokens do not expire unless explicitly revoked by the user or if they remain unused for 6 months.

- For security reasons, redirect URIs must use HTTPS as their protocol.

- When making API requests, include the access token in the Authorization header: `Authorization: Zoho-oauthtoken {access_token}`
...
- The `location` parameter in the authorization response indicates the user's datacenter. Use this to determine which datacenter to use for token and API requests.
