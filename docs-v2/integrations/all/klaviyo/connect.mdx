...
#### Step 1: Finding Klaviyo API Key
1. Log into your Klaviyo account and navigate to your account’s `Settings` page and select `API keys`.
2. Under the `Private API Key`s` section, select `Create Private API Key`.
...
Important: For your account's security, your private API keys should never be used with Client endpoints,
exposed in client-side code, or made accessible from public repositories.
...
To authenticate using your API credentials, navigate to the Klaviyo authentication form.
