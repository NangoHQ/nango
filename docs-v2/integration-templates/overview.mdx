...
Nango lets you construct [**any** integration](/core-concepts) with [over 100 supported APIs](/integrations/overview).
...
<Info>
  Pre-requisite: Set up the Nango CLI & `nango-integration` folder (see [syncs guide - Step 1](/guides/sync#step-1-setup-the-nango-cli-and-nango-integrations-folder))
</Info>
...
2.  If you wish, extend and customize the template to suit your needs (see [syncs guide](/guides/sync)).

3.  Test the sync with `nango dryrun -h` ([docs](/guides/sync#dry-run-your-sync))

4.  Deploy your integration with `nango deploy [dev|prod]` ([docs](/guides/sync#step-3-deploy-a-sync))
