...
Syncs are defined by you and live in your own repository: They are version-controlled with the rest of your codebase.

We have [examples](/nango-sync/examples) for many popular use cases to help you get started and we have tooling to help you write and test syncs locally (see the [guide on writing syncs](/nango-sync/guides/sync)).

When your Syncs are ready, you deploy them with a CLI command to Nango: Nango then runs them for you in production and handles scaling, scheduling, retries, data caching, de-duplication etc.
...
The Nango Proxy makes this easy and lets you benefit from authorization, retries, rate-limit handling, monitoring, etc.
...
You can find more information and contribute API Configurations [here](/contribute/api-configuration)

### Integrations[](#integrations)

Developers create Integrations, based on [API Configurations](/core-concepts#api-configurations), to work with external APIs from their app.
...
- a Sync typescript file in the `nango-integrations` folder

Syncs have 'runs' or 'jobs' that are executed periodically. They can be managed (stopped, paused, resync, etc.) from the Dashboard and in code.
