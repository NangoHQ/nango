...
<Update label="27 August 2025"  tags={["Breaking changes", "API"]}>

  ## EOL for Connection End User Auto-Replication

Currently connection's end user are automatically replicated to the Nango platform based on the `end_user_id` field.
This behavior was confusing for some customers and was preventing them from declaring multiple organizations for the same user without having to manually concatenate the `end_user_id` field.
Because the behavior was most of time undesired, we will stop replicating end user fields across connections.

After the transition period, an **end user will be tied to a single connection.**

  ### 🗓️ Timeline

  - **August 27, 2025**: Deprecation warning.
...
<Update label="22 August 2025"  tags={["Breaking changes", "API", "Scripts"]}>

  ## Action output limit

Nango actions currently have a documented output limit of 10MB. We are further restricting that limit to 2MB.
Instead of using an action for large payloads like this, we encourage usage of the Nango proxy.
Starting today we will warn users where we detect an output larger than 2MB and later we will throw an error where outputs are larger than 2MB.

  ### 🗓️ Timeline

  - **August 22, 2025**: We will start warning users where we detect an output larger than 2MB.
  - **September 22, 2025**: We will throw an error where outputs are larger than 2MB.

  ### 🧐 Am I impacted?

  - If you use custom actions and return large payloads

  ### 💡 What should I do?

  - Use the Nango proxy instead of custom actions for large payloads.

</Update>


<Update label="August 2025" description="v0.66.2" tags={["Breaking changes", "API", "CLI"]}>
...
<Update label="21 July 2025" description="v0.64.0" tags={["Breaking changes","CLI", "Scripts"]}>
...
By upgrading to `>=0.64.0` and running `nango compile`, the CLI will update the dependency automatically; otherwise, you need to install zod: `4.0.5`
  ...
  - **July 21, 2025**: We will start deprecating the `nango.yaml` format.
