...
provider-slug: # Shorthand for the provider, ideally the provider's name. Must be unique. Kebab case.
    ...
    # Proxy
    proxy:
        base_url: https://gitlab.example.com/api/v4 # Presets the base URL when using the Proxy (supports string interpolation, e.g. Salesforce).
...
<Note>
    You can configure some [Connection Metadata](/nango-auth/configuration), which is additional metadata that you want to capture during the OAuth flow and store in the _connection_.
</Note>
