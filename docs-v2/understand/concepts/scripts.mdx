...
Upon deployment, integration scripts are stored in blob storage. The Nango runner loads and executes these integration scripts as needed ([architecture](/understand/concepts/architecture)).
...
When developing a new integration script, your first step should be to define it in the `nango.yaml` file. Following this, you can use the [CLI](#the-nango-cli) to generate the necessary integration script scaffolding ([step-by-step guide](/guides/customize/create-a-custom-integration)).
...
The Nango Command-Line Interface (CLI), installed via `npm`, lets you generate, compile, test, and deploy integration scripts & configuration. For a detailed installation guide, refer to the [step-by-step guide](/guides/customize/setup).
