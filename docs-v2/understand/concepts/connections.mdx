...
Connections also maintain the raw credentials as returned by the external API, which may include additional API-specific information. These raw credentials can be viewed or fetched through the [Nango UI](understand/concepts/ui) and [API](reference/overview).
...
# Conclusion
Connections in Nango are not just links between customers and external APIs; they are the foundation of personalized, secure, and efficient API integration. By enabling dynamic metadata editing and distinguishing between the roles of sync and action scripts, Nango offers a flexible framework for building and managing integrations that respond adeptly to both scheduled tasks and real-time user interactions.