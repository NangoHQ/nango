...
Fetches records from the Nango cache by id. Records are returned as a `Map` keyed the requested ids. Any records not found will not be present in the resulting map.

This is especially useful in cases where a webhook sync script receives a partial record and needs to fetch the existing data to merge.
