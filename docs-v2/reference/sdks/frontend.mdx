...
You store end-user credentials with the `nango.authentication` method. It creates a [connection](/understand/concepts/connections) in Nango.
...
```js
const result = await nango.authentication('<INTEGRATION-ID>').catch((error) => {
...
});
```
...
<Expandable>
  ...
  <ResponseField name="connectionToken" type="string">
