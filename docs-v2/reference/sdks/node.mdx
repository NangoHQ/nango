...
### Bulk set connection metadata

Set custom metadata for multiple connections (overrides existing metadata).

```js
await nango.setMetadata('<INTEGRATION-ID>', ['CONNECTION-ID-1', 'CONNECTION-ID-2'], { 'CUSTOM_KEY1': 'CUSTOM_VALUE1' });
```
...
### Bulk edit connection metadata

Edit custom metadata for the connection. Only overrides specified properties, not the entire metadata.

```js
await nango.updateMetadata('<INTEGRATION-ID>', ['CONNECTION-ID-1', 'CONNECTION-ID-2'], { 'CUSTOM_KEY1': 'CUSTOM_VALUE1' });
```
...
<Expandable>
    ...
    <ResponseField name="connectionIds" type="string[]" required>
        The connection IDs.
    </ResponseField>
