...
If you want to enable logs, you will need to first:
- Host an elasticsearch cluster in the way you want
- Update your environment variables, set `NANGO_LOGS_ENABLED=true` and add the appropriate values in `NANGO_LOGS_ES_*`

### Hosting

To host elasticsearch you have multiple solutions:

- Locally: uncomment the service inside [`docker-compose.yml`](https://github.com/NangoHQ/nango/blob/master/docker-compose.yaml#L63) and run `docker-compose up -d`
- ElasticCloud: use the minimal installation on Standard tier, about 20€/mo [elastic.co](https://www.elastic.co/)
- Render: deploy a free instance [doc](https://docs.render.com/deploy-elasticsearch)
