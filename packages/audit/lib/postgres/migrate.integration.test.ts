...
async function insert(id: string, accountId: number, occurredAt: string, onConflict = ''): Promise<void> {
    await knex.raw(`INSERT INTO ${table} (event, occurred_at) VALUES (?, ?) ${onConflict}`, [event(id, accountId, occurredAt), occurredAt]);
}
