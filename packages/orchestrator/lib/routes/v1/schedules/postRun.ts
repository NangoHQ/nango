...
export type PostScheduleRun = Endpoint<{
    ...
    Body: {
        scheduleName: string;
        payloadMerge?: Record<string, JsonValue> | undefined;
    };
...
const bodySchema = z
    .object({
        ...
        payloadMerge: z.record(z.string(), jsonSchema).optional()
    })
    .strict();
...
const handler = (scheduler: Scheduler) => {
    return async (_req: EndpointRequest, res: EndpointResponse<PostScheduleRun>) => {
        const schedule = await scheduler.immediate({
            ...
            ...(res.locals.parsedBody.payloadMerge ? { payloadMerge: res.locals.parsedBody.payloadMerge } : {})
        });
        if (schedule.isErr()) {
