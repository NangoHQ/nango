export const sql = [
    `
    CREATE TABLE IF NOT EXISTS {database:Identifier}.daily_proxy
    (
        day              Date,
        account_id       Int64,
    ...
    `,
    `
    CREATE MATERIALIZED VIEW IF NOT EXISTS {database:Identifier}.daily_proxy_mv
    TO {database:Identifier}.daily_proxy AS
    SELECT
        toDate(ts)                          AS day,
        account_id,
