...
export default class Pizzly {
    ...
    public connect(integrationName: string, connectionId: string): Promise<any> {
        const url = new URL(`/oauth/connect/${integrationName}?connectionId=${connectionId}`, this.hostBaseUrl).href;
