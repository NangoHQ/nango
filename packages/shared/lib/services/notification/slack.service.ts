...
import { baseUrl } from '@nangohq/utils/dist/environment/detection.js';
...
class SlackService {
    ...
    async reportFailure(
        ...
        const payload: NotificationPayload = {
            content: `*${syncName}* (${flowType.toLowerCase()}) is failing for ${count} ${connection}. Read <${baseUrl}/${envName}/activity?activity_log_id=${originalActivityLogId}&script=${syncName}&date=${dateString}|logs>.`,
            status: 'open',
    ...
    async reportResolution(
        ...
        if (connectionCount === 0) {
            payloadContent = `[Resolved] *${syncName}* (${syncType.toLowerCase()}) failed. Read <${baseUrl}/${envName}/activity?activity_log_id=${originalActivityLogId}|logs>.`;
        } else {
            const count = connectionCount;
            const connection = count === 1 ? 'connection' : 'connections';
            payloadContent = `*${syncName}* (${syncType.toLowerCase()}) is failing for ${count} ${connection}. Read <${baseUrl}/${envName}/activity?activity_log_id=${originalActivityLogId}|logs>.`;
        }
