...
export class SlackService {
    ...
    async reportFailure(nangoConnection: NangoConnection, name: string, type: string, originalActivityLogId: string, environment_id: number, provider: string) {
        ...
        try {
            ...
            if (actionResponse.isOk()) {
                await logCtx.info(`The action ${this.actionName} was successfully triggered for the ${flowType}`, { payload, name });
                await logCtx.success();
            } else {
                await logCtx.error(`The action ${this.actionName} failed to trigger for the ${flowType}`, { error: actionResponse.error, payload, name });
                await logCtx.failed();
            }
        } catch (error) {
    ...
    async reportResolution(
        ...
        try {
            ...
            if (actionResponse.isOk()) {
                await logCtx.info(`The action ${this.actionName} was successfully triggered for "${type}"`, { payload, syncName });
                await logCtx.success();
            } else {
                await logCtx.error(`The action ${this.actionName} failed to trigger for "${type}"`, { error: actionResponse.error, payload, syncName });
                await logCtx.failed();
            }
        } catch (error) {
            await logCtx.error('Failed to trigger slack notification', { error });
