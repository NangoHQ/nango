...
export function isS3Configured(envs: ObjectStoreEnvs): boolean {
    return Boolean((envs.AWS_INTEGRATIONS_REGION && envs.AWS_INTEGRATIONS_BUCKET_NAME) || (envs.AWS_REGION && envs.AWS_BUCKET_NAME));
}

export function isGcsConfigured(envs: ObjectStoreEnvs): boolean {
    return Boolean(envs.GCS_INTEGRATIONS_BUCKET_NAME);
}

export function isAzureConfigured(envs: ObjectStoreEnvs): boolean {
 ...
 */
export function resolveObjectStoreConfig(envs: ObjectStoreEnvs): ObjectStoreConfig {
