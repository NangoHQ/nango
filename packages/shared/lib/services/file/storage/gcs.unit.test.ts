...
function memoryBucket({ failDeletes = new Set<string>() }: { failDeletes?: Set<string> } = {}): { bucket: GcsBucketClient; files: Map<string, string> } {
...
describe(GcsObjectStore, () => {
    ...
    it('continues remaining deletes and reports every failed key', async () => {
        const { bucket, files } = memoryBucket({ failDeletes: new Set(['fail.js', 'also-fail.js']) });
        const store = new GcsObjectStore(bucket);
