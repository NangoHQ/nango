...
describe('Account service', () => {
    ...
    it('should retrieve account context by secretKey', async () => {
        ...
        const secret = (await secretService.getDefaultSecretForEnv(db.readOnly, environment!.id)).unwrap();

        const bySecretKey = await accountService.getAccountContext({ secretKey: secret.secret });
