...
class AccountService {
    ...
    async getAccountContext(
        ...
        if ('secretKey' in opts) {
            if (isSandboxApiKey(opts.secretKey)) {
                return this.getAccountContextBySandboxApiKey(opts.secretKey);
            }
    ...
    private async getAccountContextBySandboxApiKey(sandboxApiKey: string): Promise<AccountContext | null> {
        ...
        if (!verified || verified.kid !== row.auth_api_key_id) {
            return null;
        }

        try {
            // Do not fold this into the lookup CTE: the JWT kid header is untrusted until the signature checks out.
            await db
                .knex('customer_keys')
