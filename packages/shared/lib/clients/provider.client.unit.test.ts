...
describe('ProviderClient Plaud OAuth', () => {
    ...
    it.each([
        [
            ...
            () =>
                providerClient.getToken(
                    makeConfig(),
                    makeProvider(),
                    makeProvider().token_url as string,
                    'code',
                    'https://api.nango.dev/oauth/callback',
                    'verifier'
                ),
            'plaud_token_request_error'
        ],
        ['token refresh', () => providerClient.refreshToken(makeProvider(), makeConfig(), makeConnection()), 'plaud_refresh_token_request_error']
    ] as const)('maps a failed %s to an HTTP 400 Nango error', async (_operation, request, type) => {
        vi.spyOn(axiosInstance, 'post').mockRejectedValue(new Error('Plaud rejected the token request'));

        const error = await request().catch((err: unknown) => err);

        expect(error).toBeInstanceOf(NangoError);
        expect(error).toMatchObject({ type, status: 400 });
    });
});
