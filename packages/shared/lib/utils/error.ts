...
export class NangoError extends NangoInternalError {
    ...
    constructor(type: string, payload = {}, status?: number, additional_properties?: Record<string, JsonValue>) {
        ...
        switch (type) {
            ...
            case 'plaud_token_request_error':
                this.status = 400;
                this.message = `The Plaud API returned an error when trying to request an access token. Please try again later.`;
                if (this.payload) {
                    this.message += ` Error: ${typeof this.payload === 'string' ? this.payload : JSON.stringify(this.payload)}`;
                }
                break;

            case 'plaud_refresh_token_request_error':
                this.status = 400;
                this.message = `The Plaud API returned an error when trying to refresh the access token. Please try again later.`;
                if (this.payload) {
                    this.message += ` Error: ${typeof this.payload === 'string' ? this.payload : JSON.stringify(this.payload)}`;
                }
                break;
