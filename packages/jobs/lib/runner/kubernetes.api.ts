...
export class Kubernetes {
    ...
    protected readonly appsApi: k8s.AppsV1Api;
    protected readonly coreApi: k8s.CoreV1Api;
    protected readonly networkingApi: k8s.NetworkingV1Api;
    protected readonly defaultNamespace: string;
    protected readonly namespacePerRunner: boolean;
    protected readonly jobsNamespace: string;
    ...
    protected async ensureNamespace(namespace: string): Promise<Result<void>> {
    ...
    protected async createDeployment(node: Node, name: string, namespace: string, runnerUrl: string): Promise<Result<void>> {
        let noDisruptSpec = {};
    ...
    protected async createService(_node: Node, name: string, namespace: string): Promise<Result<void>> {
    ...
    protected async createNetworkPolicies(namespace: string, nodeId: number): Promise<Result<void>> {
    ...
    protected async deleteNetworkPolicies(namespace: string, nodeId: number): Promise<void> {
        try {
    ...
    protected getServiceName(node: Node): string {
        return `${node.routingId}-${node.id}`;
    }

    protected getNamespace(node: Node): string {
        if (this.namespacePerRunner) {
    ...
    protected getRunnerUrl(node: Node): string {
        const name = this.getServiceName(node);
        if (this.namespacePerRunner) {
            const namespace = this.getNamespace(node);
    ...
    protected getEnvironmentVariables(node: Node, runnerUrl: string): k8s.V1EnvVar[] {
        return [
    ...
    protected isProfilingEnabled(node: Node): boolean {
    ...
    protected getResourceLimits(node: Node): { requests: { cpu: string; memory: string }; limits: { cpu: string; memory: string } } {
