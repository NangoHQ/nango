 ...
 *
 * `makeAcc` is called once per chunk to produce a fresh accumulator.
 ...
 * );
 */
export function chunk<T, Acc>(items: T[], makeAcc: () => Acc, reduce: (acc: Acc, item: T) => Acc, wouldOverflow: (acc: Acc, item: T) => boolean): T[][] {
    const chunks: T[][] = [];
    let currentChunk: T[] = [];
    let acc = makeAcc();
