...
describe(`POST ${endpoint}`, () => {
    ...
    describe('deploy', () => {
        ...
        it('should delete a disabled action when a same-named sync is still deployed', async () => {
            ...
            const { nangoYamlBody: _nangoYamlBody, ...confirmationBody } = secondDeployBody;

            const confirmation = await api.fetch('/sync/deploy/confirmation', {
                method: 'POST',
                token: secret.secret,
                body: {
                    ...confirmationBody,
            ...
            isSuccess(confirmation.json);
            expect(confirmation.res.status).toBe(200);
            expect(confirmation.json.deletedActions).toStrictEqual([
                {
                    name: 'shared',
                    providerConfigKey: 'unauthenticated'
                }
