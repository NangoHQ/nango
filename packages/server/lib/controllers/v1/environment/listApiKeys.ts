...
import type { EnvironmentApiKeyScope, ListApiKeys } from '@nangohq/types';
...
export const listApiKeys = asyncWrapperWithEnvironment<ListApiKeys>(async (req, res) => {
    ...
    const keysResult = await customerKeyService.getApiKeys(db.knex, { type: 'environment', environmentId: environment.id });
    if (keysResult.isErr()) {
