...
import { configService, connectionService, getApiUrl, getSyncConfigRaw, secretService } from '@nangohq/shared';
...
import type { NangoProps, PostSfDryRun } from '@nangohq/types';
...
export const postSfDryRun = asyncWrapper<PostSfDryRun>(async (req, res) => {
    ...
    try {
        ({ bundledJs } = await invokeCompiler({
    ...
    // Look up sync config for metadata (optional — may not exist if function is brand new)
    ...
    const nangoProps: NangoProps = {
        ...
        syncId: body.function_type === 'sync' ? `sf-dryrun-${Date.now()}` : undefined,
        ...
        track_deletes: syncConfig?.track_deletes ?? false,
        syncConfig: syncConfig ?? (undefined as any),
        debug: false,
        logger: { level: 'off' },
