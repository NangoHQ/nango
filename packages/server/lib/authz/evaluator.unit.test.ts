...
describe('StaticEvaluator', () => {
    ...
    describe('production_support', () => {
        ...
        it('should deny environment creation', async () => {
            await expect(evaluator.evaluate({ role: 'production_support' }, { action: 'create', resource: 'environment', scope: 'global' })).resolves.toBe(
                false
            );
        });
        ...
        it('should deny production secrets/credentials', async () => {
            await expect(evaluator.evaluate({ role: 'production_support' }, { action: 'read', resource: 'secret_key', scope: 'production' })).resolves.toBe(
                false
            );
            await expect(
                evaluator.evaluate({ role: 'production_support' }, { action: 'read', resource: 'connection_credential', scope: 'production' })
            ).resolves.toBe(false);
        });

        it('should allow production read operations', async () => {
            await expect(evaluator.evaluate({ role: 'production_support' }, { action: 'read', resource: 'integration', scope: 'production' })).resolves.toBe(
                true
            );
            await expect(evaluator.evaluate({ role: 'production_support' }, { action: 'read', resource: 'connection', scope: 'production' })).resolves.toBe(
                true
            );
            await expect(evaluator.evaluate({ role: 'production_support' }, { action: 'read', resource: 'environment', scope: 'production' })).resolves.toBe(
                true
            );
            await expect(evaluator.evaluate({ role: 'production_support' }, { action: 'read', resource: 'log', scope: 'production' })).resolves.toBe(true);
        });
        ...
        it('should allow non-prod everything', async () => {
            ...
            await expect(evaluator.evaluate({ role: 'production_support' }, { action: 'read', resource: 'secret_key', scope: 'non-production' })).resolves.toBe(
                true
            );
        });
    });
    ...
    describe('development_full_access', () => {
        it('should deny all production access', async () => {
            await expect(
                evaluator.evaluate({ role: 'development_full_access' }, { action: 'read', resource: 'integration', scope: 'production' })
            ).resolves.toBe(false);
            await expect(
            ...
            await expect(
                evaluator.evaluate({ role: 'development_full_access' }, { action: 'read', resource: 'secret_key', scope: 'production' })
            ).resolves.toBe(false);
            await expect(
                evaluator.evaluate({ role: 'development_full_access' }, { action: 'read', resource: 'connection_credential', scope: 'production' })
            ).resolves.toBe(false);
        });
        ...
        it('should deny environment creation', async () => {
            await expect(evaluator.evaluate({ role: 'development_full_access' }, { action: 'create', resource: 'environment', scope: 'global' })).resolves.toBe(
                false
            );
        });
        ...
        it('should allow non-prod everything', async () => {
            ...
            await expect(
                evaluator.evaluate({ role: 'development_full_access' }, { action: 'read', resource: 'secret_key', scope: 'non-production' })
            ).resolves.toBe(true);
            await expect(
                evaluator.evaluate({ role: 'development_full_access' }, { action: 'read', resource: 'connection_credential', scope: 'non-production' })
            ).resolves.toBe(true);
        });
        ...
        it('should allow account-level reads (plans, user, getting-started)', async () => {
            await expect(evaluator.evaluate({ role: 'development_full_access' }, { action: 'read', resource: 'plan', scope: 'global' })).resolves.toBe(true);
        });
    });
    ...
    describe('unknown role', () => {
        it('should deny everything', async () => {
            ...
            await expect(evaluator.evaluate({ role: 'unknown' }, { action: 'read', resource: 'integration', scope: 'non-production' })).resolves.toBe(false);
        });
    });
});
