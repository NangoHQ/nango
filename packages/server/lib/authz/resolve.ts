...
/**
 * The permission's tier decides the plane. The environment in locals supplies the id when there is
 * one; roles only ever select on the tier, so its absence is not a gap.
 */
...
export function buildPermissions(role: Role, plan?: { has_rbac: boolean } | null): AllowedPermissions {
    // Account-agnostic: the question is what the role may do, not where.
    const accountId = 0;
