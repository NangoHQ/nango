...
const secrets = { 'tenant-a': 'connection-secret-a', 'tenant-b': 'connection-secret-b', 'tenant:123': 'secret:with:colons' };
...
function auth(value = `nango:${secrets['tenant-a']}`) {
...
const body = { event: 'new ticket logged', ticket: { id: 123 } };
const query = { nangoConnectionId: 'tenant-a' };

describe('Halo PSA webhook routing', () => {
    it.each([undefined, {}, { nangoConnectionId: '' }])('rejects a missing connection ID before lookup: %j', async (query) => {
        const { nango, lookup, execute } = setup();
        const result = await route(nango, auth(), body, '', query);
        expect(result.isErr()).toBe(true);
        if (result.isErr()) {
            expect(result.error).toBeInstanceOf(NangoError);
            expect(result.error).toMatchObject({ type: 'webhook_missing_connection_id', status: 400 });
        }
    ...
    it('rejects an empty password', async () => {
        ...
        expectUnauthorized(await route(nango, auth('nango:'), body, '', query), 'webhook_invalid_signature');
        expect(execute).not.toHaveBeenCalled();
    });
    ...
    it.each(Object.entries(secrets))('authenticates %s only with its own secret and preserves the payload', async (connectionId, secret) => {
        ...
        expectUnauthorized(await route(nango, auth(`nango:${otherSecret}`), body, '', { nangoConnectionId: connectionId }), 'webhook_invalid_signature');
        expect(execute).not.toHaveBeenCalled();
    });
    ...
    it.each([
        ...
        [auth(`tenant-a:${secrets['tenant-a']}`).authorization, 'webhook_invalid_signature'],
        [auth('other:password').authorization, 'webhook_invalid_signature']
    ])('rejects malformed credentials before connection lookup: %s', async (authorization, errorType) => {
        const { nango, lookup, execute } = setup();
        expectUnauthorized(await route(nango, { authorization }, body, '', query), errorType);
        expect(lookup).not.toHaveBeenCalled();
        expect(execute).not.toHaveBeenCalled();
    });
    ...
    it.each([null, '', 123, 'another-tenant-secret'])('rejects missing, invalid, or mismatched connection secrets: %s', async (secret) => {
        ...
        expectUnauthorized(await route(nango, auth(), body, '', query), 'webhook_invalid_signature');
        expect(execute).not.toHaveBeenCalled();
    });
    ...
    it('accepts colons in passwords', async () => {
        ...
        expect((await route(nango, auth('nango:secret:with:colons'), body, '', query)).isOk()).toBe(true);
    });
});
