...
class CacheService {
    ...
    async set(key: string, value: Record<any, any>): Promise<string> {
        ...
        if (cache) {
            return this.findByKeyQueryBuilder(key)
                .update({ key, value: JSON.stringify(value) }, ['key']);
        } else {
            return this.findByKeyQueryBuilder(key)
                .insert({ key, value: JSON.stringify(value) }, ['key']);
        }
    ...
    async delete(key: string) {
        ...
        await this.findByKeyQueryBuilder(key)
            .delete();
    }
