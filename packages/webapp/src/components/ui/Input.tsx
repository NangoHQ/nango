...
const Input = React.forwardRef<HTMLInputElement, React.ComponentProps<'input'>>(({ className, type, autoComplete, ...props }, ref) => {
    ...
    return (
        <input
            ...
            className={cn(
                'bg-surface-canvas border border-border-muted text-text-strong !text-body-medium-regular placeholder:text-text-muted focus-shadow focus:border-border-default data-[filled=true]:not-aria-invalid:border-border-strong',
                ...
                'aria-invalid:focus-error aria-invalid:border-text-danger',
                className
            )}
