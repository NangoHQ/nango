...
export const TabsTrigger: React.FC<React.ComponentProps<typeof TabsPrimitive.Trigger>> = (props) => {
    return (
        <TabsPrimitive.Trigger
            {...props}
            className={cn(
                'w-fit px-3 py-2 inline-flex items-center gap-1.5 cursor-pointer text-text-secondary !text-body-medium-medium border-b-2 border-b-transparent transition-colors hover:text-text-strong hover:border-text-tertiary data-[state=active]:text-text-strong data-[state=active]:border-text-strong focus-default',
                props.className
            )}
        />
    );
