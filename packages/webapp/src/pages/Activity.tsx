...
export default function Activity() {
    ...
    return (
        <DashboardLayout selectedItem={LeftNavBarItems.Activity} marginBottom={60}>
            <div className="relative -left-24">
                ...
                {activities && activities.length === 0 && !status && !selectedIntegration && !selectedScript && !selectedConnection && !selectedDate ? null : (
                    <div className="flex justify-between p-3 mb-6 items-center border border-border-gray rounded-md min-w-[1150px]">
                        <div className="flex space-x-10 justify-between px-2 w-full">
                            <div className="flex w-full items-center">
                                ...
                                >
                                    ...
                                    <option key="progress" value="in_progress">
                                        In Progress
                                    </option>
                            ...
                            {scripts.length > 0 && (
                                <div className="flex w-full items-center">
                                    ...
                                    {selectedScript && (
                                        <XCircleIcon
                                            onClick={() => onRemoveFilter(setSelectedScript, 'script')}
                        ...
                        <div className="flex">
                            {Number(offset) >= limit * 3 && <ChevronsLeft onClick={resetOffset} className="flex stroke-white cursor-pointer mr-3" size="16" />}
                            <span
                                onClick={decrementPage}
                                className={`flex ${Number(offset) - limit >= 0 ? 'cursor-pointer hover:bg-gray-700' : ''} h-8 mr-2 rounded-md px-3 pt-1.5 text-sm text-white bg-gray-800`}
                            >
                                                                ...
                                                                <Link
                                                                    to={
                                                                        activity.provider === null
                                                                            ? `/${env}/integrations`
                                                                            : `/${env}/integration/${activity.provider_config_key}`
                                                                    }
            ...
            <Helmet>
                <style>{'::-webkit-calendar-picker-indicator { filter: invert(1); }'}</style>
            </Helmet>
        </DashboardLayout>
    );
}
