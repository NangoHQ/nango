...
export default function Activity() {
    ...
    const { error } = useSWR(`/api/v1/activity?env=${env}&limit=${limit}&offset=${offset}` +
            ...
            <div className="relative -left-24">
                ...
                {activities && activities.length === 0 && (!status && !selectedIntegration && !selectedScript && !selectedConnection && !selectedDate) ? null : (
                    <div className="flex justify-between p-3 mb-6 items-center border border-border-gray rounded-md min-w-[1150px]">
                        ...
                        <div className="flex">
                            {Number(offset) >= limit * 3 && (
                                <ChevronsLeft onClick={resetOffset} className="flex stroke-white cursor-pointer mr-3" size="16" />
                            )}
                            <span onClick={decrementPage} className={`flex ${Number(offset) - limit >= 0 ? 'cursor-pointer hover:bg-gray-700' : ''} h-8 mr-2 rounded-md px-3 pt-1.5 text-sm text-white bg-gray-800`}>
                            ...
                            <span onClick={incrementPage} className={`flex ${activities && activities?.length < limit ? '' : 'cursor-pointer hover:bg-gray-700'} h-8 rounded-md px-3 pt-1.5 text-sm text-white bg-gray-800`}>
