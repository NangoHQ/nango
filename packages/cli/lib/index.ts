...
// Handle shell completion early (before any other CLI processing)
    ...
    class NangoCommand extends Command {
        override createCommand(name: string) {
            ...
            cmd.hook('preAction', async function (this: Command, actionCommand: Command) {
                ...
                // Skip upgrade check for completion command (needs to be fast)
                const commandName = actionCommand.name();
                if (!isCI && commandName !== 'completion') {
...
`
    ...
    program
        ...
        .action(async function (this: Command, template: string) {
            ...
            const success = await cloneTemplate({ fullPath, template, debug, force, autoConfirm });
            if (!success) {
                process.exitCode = 1;
            }
        });
    ...
    program
        ...
        .action(function (this: Command) {
            ...
            if (action === 'install') {
                ...
                // Called by shell for completions - already handled above
            }
        });
