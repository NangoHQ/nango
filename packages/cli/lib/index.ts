...
program
    ...
    .option('--integration-id [optionalProviderConfigKey]', 'Optional: Deploy all scripts related to a specific integration.')
...
// admin only commands
// TODO: Add option to deploy specific integration if needed
program
...
program
    ...
    .option('-i, --integration-id [optionalProviderConfigKey]', 'Optional: Deploy all scripts related to a specific integration/provider config key.')
    .action(async function (this: Command, environment: string) {
        const { debug, nangoRemoteEnvironment, optionalProviderConfigKey } = this.opts();
        const fullPath = process.cwd();
        await deployService.internalDeploy({ fullPath, environment, debug, options: { env: nangoRemoteEnvironment || 'prod', optionalProviderConfigKey } });
    });
