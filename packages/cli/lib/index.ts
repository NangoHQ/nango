...
import { handleCompletion, installCompletion, uninstallCompletion } from './services/completion.service.js';
...
import { pull } from './services/pull.service.js';
...
// Handle shell completion requests early, before any other initialization
// This avoids upgrade prompts and other side effects during tab completion
if (process.env['COMP_LINE'] !== undefined) {
    handleCompletion()
        .then(() => process.exit(0))
        .catch(() => process.exit(0));
} else {
    // Normal CLI execution continues below
    runCLI();
}
    ...
    class NangoCommand extends Command {
        override createCommand(name: string) {
            ...
            cmd.hook('preAction', async function (this: Command, actionCommand: Command) {
                ...
                // Skip upgrade check for completion command
                if (!isCI && actionCommand.name() !== 'completion') {
...
`
    ...
    program
        ...
        .description('Pull integration templates from the integration-templates repository')
        .argument('<template>', 'Template path: "github", "github/actions", or "github/actions/list-repos"')
        ...
        .action(async function (this: Command, template: string) {
            ...
            if (!precheck.isNango) {
                console.error(chalk.red(`Not inside a Nango folder. Run 'nango init' first.`));
                process.exitCode = 1;
                return;
            }

            if (!precheck.isZeroYaml) {
                console.error(chalk.red(`Pull is only supported for zero-yaml projects. Please migrate first with 'nango migrate-to-zero-yaml'.`));
                process.exitCode = 1;
                return;
            }

            const success = await pull({ fullPath, template, debug, force, autoConfirm });
            if (!success) {
                process.exitCode = 1;
            }
        });
    ...
    program
        .command('completion')
        .description('Install or uninstall shell tab completion for nango commands')
        .argument('<action>', 'Action to perform: install or uninstall')
        .action(function (this: Command, action: string) {
            if (action === 'install') {
                installCompletion();
            } else if (action === 'uninstall') {
                uninstallCompletion();
            } else {
                console.log(chalk.red(`Usage: nango completion <install|uninstall>`));
                process.exitCode = 1;
            }
        });
