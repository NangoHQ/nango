...
export function loadYamlAndGenerate({ fullPath, debug = false }: { fullPath: string; debug?: boolean }): NangoYamlParsed | null {
    ...
    generateNangoJson({ parsed: parsing.value.parsed!, fullPath, debug });
...
export function generateNangoJson({ fullPath, parsed, debug }: { fullPath: string; parsed: NangoYamlParsed; debug: boolean }): void {
    const exportPath = path.resolve(fullPath, '.nango');
    if (debug) {
        printDebug(`Generating .nango/nango.json in ${exportPath}`);
    }
