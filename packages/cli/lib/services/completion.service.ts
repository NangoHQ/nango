...
interface CompletionCache {
    integrations?: CacheEntry<string[]>;
    scripts?: Record<string, CacheEntry<string[]>>;
}
...
async function fetchIntegrations(): Promise<string[]> {
    const cache = (state.get('completionCache') as CompletionCache | undefined)?.integrations;
    ...
    // Update cache
    const currentCache = (state.get('completionCache') as CompletionCache) || {};
...
async function fetchScripts(integration: string, type: string): Promise<string[]> {
    ...
    const cache = (state.get('completionCache') as CompletionCache | undefined)?.scripts?.[cacheKey];
...
/**
 * Get completions for the clone command
 */
async function getCloneCompletions(partial: string): Promise<CompletionResult[]> {
...
export async function handleCompletion(): Promise<void> {
    ...
    try {
        // Check if we're completing the "clone" command
        const cloneIndex = words.indexOf('clone');

        if (cloneIndex === -1) {
            // Not completing clone command, return empty
            return;
        }

        // Get the partial word being completed (after "clone")
        const partial = words[cloneIndex + 1] || '';

        const completions = await getCloneCompletions(partial);
