...
export default async function fetchData(nango: NangoSync): Promise<void> {
    ...
    for await (const invoices of paginate<{ id: string }>({ nango, proxyConfig })) {
        await nango.log('Listed payments', { total: invoices.length });

        const mappedPayments: NetsuitePayment[] = [];
        for (const paymentLink of invoices) {
            const payment: NSAPI_GetResponse<NS_Payment> = await nango.get({
                endpoint: `/customerpayment/${paymentLink.id}`,
                retries
            });
            if (!payment.data) {
                await nango.log('Invoice not found', { id: paymentLink.id });
                continue;
            }
            ...
            mappedPayments.push({
                ...
                amount: payment.data.payment ? Number(payment.data.payment) : null,
                currency: payment.data.currency?.id || null,
