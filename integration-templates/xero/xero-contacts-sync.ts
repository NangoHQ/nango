...
export default async function fetchData(nango: NangoSync): Promise<void> {
  ...
  await nango.log(
    `Last sync date - type: ${typeof nango.lastSyncDate} JSON value: ${JSON.stringify(
      nango.lastSyncDate
    )}`
  );
  ...
  if (nango.lastSyncDate) {
    ...
    config.headers["If-Modified-Since"] = nango.lastSyncDate
      .toISOString()
      .replace(/\.\d{3}Z$/, ""); // Returns yyyy-mm-ddThh:mm:ss
  }
  ...
  do {
    ...
    if (nango.lastSyncDate) {
      const archivedContacts = contacts.filter(
        (x: any) => x.ContactStatus === "ARCHIVED"
      );
