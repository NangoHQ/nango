...
export default async function runAction(
  ...
  if (invalidContacts.length > 0) {
    throw new Error(
      `Some contacts are missing the id property, which is mandatory to update contacts. Affected contacts:\n${JSON.stringify(
        invalidContacts,
        null,
        4
      )}`
...
function mapContactToXero(contact: Contact) {
  ...
  if (
    ...
    var address: any = { AddressType: "POBOX" };
...
function mapXeroContact(xeroContact: any): Contact {
  ...
  var streetAddress = xeroContact.Addresses.filter(
    (x: any) => x.AddressType === "POBOX"
  )[0];
  var defaultPhone = xeroContact.Phones.filter(
    (x: any) => x.PhoneType === "DEFAULT"
  )[0];

  streetAddress = streetAddress ? streetAddress : {};
  defaultPhone = defaultPhone ? defaultPhone : {};

  var formattedPhoneNumber: any = null;
