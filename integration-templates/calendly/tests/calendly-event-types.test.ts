...
const { default: batchSaveDataRaw } = await import(`../mocks/event-types/batchSave.json`);
batchSaveData = batchSaveDataRaw;

let batchDeleteData: any = null;

const { default: batchDeleteDataRaw } = await import(`../mocks/event-types/batchDelete.json`);
...
describe('calendly event-types tests', () => {
    ...
    if (paginateGet || paginatePost || paginatePatch || paginatePut || paginateDelete) {
        nangoMock.paginate = vi.fn().mockImplementation(async function* (args: any) {
            ...
            switch (method) {
                case 'GET': {
                    const { default: paginateDataGet } = await import(`../mocks/paginate/get/event-types${endpoint}.json`, { assert: { type: 'json' } });
                    paginateGet = paginateDataGet;
                    yield paginateGet;
                    break;
                }
                case 'POST': {
                    const { default: paginateDataPost } = await import(`../mocks/paginate/post/event-types${endpoint}.json`, { assert: { type: 'json' } });
                    paginatePost = paginateDataPost;
                    yield paginatePost;
                    break;
                }
                case 'PATCH': {
                    const { default: paginateDataPatch } = await import(`../mocks/paginate/patch/event-types${endpoint}.json`, { assert: { type: 'json' } });
                    paginatePatch = paginateDataPatch;
                    yield paginatePatch;
                    break;
                }
                case 'PUT': {
                    const { default: paginateDataPut } = await import(`../mocks/paginate/put/event-types${endpoint}.json`, { assert: { type: 'json' } });
                    paginatePut = paginateDataPut;
                    yield paginatePut;
                    break;
                }
                case 'DELETE': {
                    const { default: paginateDataDelete } = await import(`../mocks/paginate/delete/event-types${endpoint}.json`, { assert: { type: 'json' } });
