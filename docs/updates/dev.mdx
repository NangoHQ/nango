...
<Update label="April 27, 2026">
  ## Sync and action runtime migration to serverless execution

  ### ✨ What's changing

  We are migrating all sync and action executions to our serverless AWS Lambda runtime, and introducing a **10-minute execution limit** per run.
  ...
  Lambda has a hard timeout, so long-running syncs and actions must checkpoint work and continue across runs instead of relying on a single long execution.

  ### 🗓️ Timeline

  - **Now**: We will start migrating accounts that do not have long-running sync executions.
  - **Next 3-6 months**: We will progressively migrate accounts with long-running syncs by assisting with migration to checkpoints.
  - We will announce milestones in this changelog and coordinate directly with impacted accounts.

  ### 🧐 Am I impacted?

  - Most accounts will not need immediate changes.
  - Accounts with syncs that regularly run longer than 10 minutes are impacted and should migrate to checkpoint-based progress. We will be reaching out directly to affected accounts to assist in the migration.

  ### 💡 What should I do?

  - Migrate all syncs to use checkpoints using the following guides: [checkpoints guide](/implementation-guides/use-cases/syncs/checkpoints), [migration guide](/implementation-guides/platform/migrations/migrate-to-checkpoints), and [deletion detection guide](/implementation-guides/use-cases/syncs/deletion-detection).

  We are also working on migration tooling/skills to speed up checkpoint adoption. If you do not want to migrate yet or cannot migrate within the timeline, please reach out and we will find a workable path with you.
...
<Update label="February 5, 2026">
  ...
  [Sync](/implementation-guides/use-cases/syncs/realtime-syncs#real-time-syncs) functions now support customizable **checkpoints**, a mechanism to save progress mid-execution and enable truly resilient incremental syncing. Instead of re-fetching your entire dataset on every run, your sync only retrieves what changed since the last checkpoint.
  ...
  This replaces `syncType: 'incremental'` and `nango.lastSyncDate`, which declared intent but left progress tracking to Nango. With checkpoints, your sync function owns the entire progress: you define the schema (timestamps, cursors, or any other composite), and call `saveCheckpoint()` after each batch. This means mid-run failures no longer cost you a full re-fetch, and you're no longer limited to timestamp-based progress tracking.

  ### 🧐 Am I impacted?

  - No breaking changes. Existing syncs without a `checkpoint` declaration continue to run as full syncs.
  - If you use `nango.lastSyncDate` for incremental syncing, we recommend migrating to checkpoints. They are more resilient, more flexible, and will be the primary supported path for incremental syncing going forward. 
  - There is no timeline for deprecating `nango.lastSyncDate`, but new improvements will only be made to checkpoints.

  ### 💡 What should I do?

  - For any sync with a large or growing dataset, define a `checkpoint` schema and call `saveCheckpoint()` after each batch.
  - If you use `nango.lastSyncDate`, follow the [migration guide](/implementation-guides/platform/migrations/migrate-to-checkpoints).
  - See the full [checkpoints documentation](/implementation-guides/use-cases/syncs/checkpoints) for details.
</Update>
...
<Update label="August 22, 2025">
  ...
  Nango actions currently have a documented output limit of 10MB. We are further restricting that limit to 2MB. Instead of using an action for large payloads like this, we encourage usage of the Nango [proxy](/implementation-guides/platform/proxy/implement-requests-proxy).
  ...
  ![](/images/action-output-size-warning.png)
