...
<Update label="February 5, 2026">
  ...
  [Sync functions](/guides/functions/syncs/sync-functions) now support customizable **checkpoints**, a mechanism to save progress mid-execution and enable truly resilient incremental syncing. Instead of re-fetching your entire dataset on every run, your sync function only retrieves what changed since the last checkpoint.
  ...
  - If you use `nango.lastSyncDate`, follow the [migration guide](/guides/platform/migrations/migrate-to-checkpoints).
  - See the full [checkpoints documentation](/guides/functions/syncs/checkpoints) for details.
</Update>
...
<Update label="December 5, 2025">
  ...
  **For new integrations:**
  ...
  - See the updated [webhook documentation](/guides/platform/webhooks-from-nango#verifying-webhooks-from-nango) for code examples
...
<Update label="August 22, 2025">
  ...
  Nango actions currently have a documented output limit of 10MB. We are further restricting that limit to 2MB. Instead of using an action for large payloads like this, we encourage usage of the Nango [proxy](/guides/platform/proxy-requests).
