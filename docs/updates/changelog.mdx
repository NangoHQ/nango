...
<Update label="BYOC self-hosting" description="September 21, 2026">
  Bring Your Own Cloud (BYOC) is now the recommended way to self-host Nango. Nango deploys and operates a dedicated instance inside your own AWS, GCP, or Azure account. Your end users' credentials, synced records, and logs never leave your cloud, and Nango handles deployments, upgrades, scaling, incident response, and support. You get every feature of Nango Cloud without operating it yourself.

  The [BYOC guide](/guides/platform/self-hosting/byoc) has the details. If you prefer to run Nango yourself, see the [Self-Managed guide](/guides/platform/self-hosting/self-managed). Both require an [Enterprise plan](https://nango.dev/pricing); the [self-hosting overview](/guides/platform/self-hosting/self-hosting#choose-your-path) compares the two.
</Update>
---
...
<Update label="Agent sessions updates" description="September 16, 2026">
  ...
  - **Clearer tool errors:** Failed tool calls return a message that says what failed and what the agent should do next, plus an error code in the result's `_meta`.
</Update>
---
...
<Update label="Google webhook verification" description="September 14, 2026">
  Nango now verifies incoming webhooks from Gmail, Google Drive, and Google Calendar before forwarding them to your app.

  - **Gmail:** With authentication enabled on your Pub/Sub subscription, Google signs each push with an OIDC token. Nango verifies its signature, issuer, expiry, and audience. Pushes without an `Authorization` header are rejected and not forwarded. Enabling authentication is now a required step in the [Gmail webhook guide](/api-integrations/google-mail/webhooks).
  - **Google Drive and Calendar:** Once you save a webhook secret on the integration, Nango checks each notification's `X-Goog-Channel-Token` header against it and rejects mismatches. Set the secret before creating channels and pass it as the channel `token`. See the [Drive](/api-integrations/google-drive/webhooks) and [Calendar](/api-integrations/google-calendar/webhooks) guides.

  **Am I impacted?** If your Gmail Pub/Sub subscription has no authentication, its notifications are no longer forwarded until you enable it. Drive and Calendar channels created without a `token` keep working until you save a webhook secret; recreate them with a `token` once you do.
</Update>
---
