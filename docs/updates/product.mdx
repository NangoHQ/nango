...
<Update label="November 2025">
  ...
  See the [customize a function template](/implementation-guides/platform/functions/customize-template) guide.

  ## Improved CLI dependency management

  We have updated how the Nango CLI handles dependencies. The CLI now natively uses npm (with full support for pnpm) to manage dependencies in your projects.

  This update forces strict versioning and directly impacts the main repository's dependencies to ensure consistency across environments. If you prefer to handle package management manually, you can bypass this behavior by passing the `--no-dependency-update` flag during CLI operations.
