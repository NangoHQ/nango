...
Connections now support **tags**: key/value strings you attach when creating a Connect session. Nango copies them onto the connection and includes them in auth webhooks, so you can tie each connection to the right user, organization, or entity in your system.

Add tags when [creating a Connect session](/implementation-guides/platform/auth/connection-tags), read them from the [auth webhook](/implementation-guides/platform/auth/connection-tags#2-read-tags-from-the-auth-webhook) and the connection object, and optionally [filter connections by tags](/implementation-guides/platform/auth/connection-tags#4-filter-connections-by-tags) or [patch them later](/implementation-guides/platform/auth/connection-tags#5-update-tags-on-an-existing-connection-optional).

In the Nango Dashboard, certain tags also improve how connections are shown: `end_user_display_name` and `end_user_email` appear in the connections list, and the email or domain is used to show the company logo on the connection detail page.


**Migration Note:** If you currently use `end_user` or `organization` on Connect sessions or in webhooks, see the [migration guide](/implementation-guides/platform/migrations/migrate-from-end-user) to move to tags. Existing connections have been backfilled with tags derived from legacy end-user fields.
...
You can still provide all arguments upfront to skip prompts (e.g., for scripting): `nango create --sync --integration my-api --name get-contacts`.
