...
It also takes `query_params` and `headers`. Nango authenticates the request from the session's connection, so authentication and hop-by-hop headers are rejected rather than forwarded.
