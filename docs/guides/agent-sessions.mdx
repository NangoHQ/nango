...
```jsonc
{
  ...
  "meta_tools": {    // Optional. nango_proxy is off unless you turn it on
...
An integration that matches no connection does not fail creation. It appears in the toolset as `connected: false`, and its tool calls fail with an error saying it is not connected.

What that error tells the agent to do next depends on whether [`nango_create_connection`](#nango_create_connection) is enabled. With it off, the agent is told it cannot fix this itself, to pass it to the user and carry on with the tools it has. With it on, the error names the tool so the agent can fetch a connect link and hand that over instead.
...
| `nango_proxy` | Makes an authenticated request to a provider API, for when no tool fits |
| `nango_create_connection` | Returns a connect link for one of the session's integrations that has no connection |

`nango_tool_search` and `nango_execute` default to `true`, `nango_proxy` and `nango_create_connection` to `false`. Set them on session creation with `meta_tools`:
...
The escape hatch for when no tool in the session covers what the agent needs. It makes an authenticated HTTP request to a provider API on the connection the session resolved for that integration, so the agent names an integration and never a connection.

```json
{ "integration": "notion", "method": "POST", "path": "/v1/search", "body": { "query": "roadmap" } }
```

It also takes `query_params` and `headers`. Nango authenticates the request from the session's connection, so credential headers are rejected rather than forwarded, including the provider's own (`x-api-key` and the like) alongside `authorization`.
...
The result is the provider's `status`, `headers` and `body`, for JSON or UTF-8 text responses up to 5 MB. Binary and larger responses are not supported, and neither is file upload. Use the [HTTP proxy](/reference/backend/http-api/proxy) for those.

### nango_create_connection

Lets the agent get the user to connect an integration that has none, instead of dead-ending on it. It takes one of the session's integrations and returns a link for the user to open.
...
```json
{
  "guidance": "Give this link to the user and wait...",
  "integration": "notion",
  "provider": "notion",
  "connect_url": "https://connect.nango.dev/?session_token=...",
  "expires_at": "2026-09-21T17:06:41.145Z"
}
```

The agent cannot follow the link itself. It passes it to the user, who authorizes it the same way as any [connect link](/guides/auth/share-connect-link) you send them yourself, and the tools for that integration work for the rest of the session once they are done.

This does not widen what the session can reach. The integration has to already be in the toolset, and one that already resolved a connection is refused rather than reconnected, so the tool only ever fills a gap the session was created with.

Nango tags the new connection with `nango/agent_session` so the session can find it. Add your own tags to have it match the selectors your next session for the same tenant will use, otherwise only this session can reach it:

```json
{ "meta_tools": { "nango_create_connection": { "enabled": true, "tags": { "organization_id": "acme" } } } }
```

You get nine tags, since the tenth is the one Nango reserves. Tags are only accepted here, not on the other meta tools.

It is off by default: it puts a connect prompt in front of your user in the agent's own words, so turning it on is a decision about your product, not just about the session.
