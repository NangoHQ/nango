...
An integration that matches no connection does not fail creation. It appears in the toolset as `connected: false`, and its tool calls fail with a missing authorization error telling the agent the integration needs connecting.

If you want the agent to be able to do something about that, enable the [`nango_create_connection`](#nango_create_connection) meta tool and it can hand the user a connect link.
