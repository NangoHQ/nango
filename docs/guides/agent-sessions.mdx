...
`nango_tool_search` and `nango_execute` default to `true`, `nango_proxy` to `false`. Set them on session creation with `meta_tools`:
...
It also takes `query_params` and `headers`. Nango authenticates the request from the session's connection, so credential headers are rejected rather than forwarded, including the provider's own (`x-api-key` and the like) alongside `authorization`. Hop-by-hop headers are rejected too.
