...
<Steps>
  <Step title="Create a Nango account">
    ...
    <Accordion title="🤖 For agents">
      ...
      If the user creates a scoped key instead, this guide needs `environment:integrations:write` (step 2) and `environment:connect_sessions:write` (step 4). See [API keys](/reference/backend/http-api/api-keys).
    </Accordion>
  </Step>
  ...
  <Step title="Create an integration">
    ...
    <Accordion title="🤖 For agents">
      Create the integration via [`POST /integrations`](/reference/backend/http-api/integration/create) (requires the `environment:integrations:write` scope).

      Find the `<PROVIDER>` slug by calling [`GET /providers`](/reference/backend/http-api/providers/list) — the `name` field of each entry is a valid slug.
      ...
      Update credentials later with [`PATCH /integrations/{uniqueKey}`](/reference/backend/http-api/integration/update).
    </Accordion>
  </Step>
  ...
  <Step title="Run the flow">
    ...
    <Accordion title="🤖 For agents">
      After the user runs the flow, confirm the connection was created with [`GET /connections`](/reference/backend/http-api/connections/list) — filter by the tags you set in step 4:
