...
- **Pre-built UI** — Embedded Connect UI with API-specific guidance, input validation, and your branding. Or [build a custom UI](/guides/auth/customize-connect-ui).
- **Credential management** — Encrypted storage, automatic refresh, retrieval at scale. Combine with [Proxy](/guides/platform/proxy/implement-requests-proxy) or [Functions](/guides/functions/overview) to avoid handling credentials directly.
...
<Steps>
  ...
  <Step title="Generate a session token (backend)">
    ...
    <Accordion title="ℹ️ Tags">
      ...
      See [Connection tags](/guides/auth/connection-tags) for more.
    </Accordion>
  ...
  <Step title="Run the flow">
    ...
    - Build with [sync functions](/guides/functions/sync-functions), [Action functions](/guides/functions/action-functions), [webhook functions](/guides/functions/webhook-functions), or any other Nango function type.
  </Step>
</Steps>
...
- [Connection tags](/guides/auth/connection-tags) - attribute connections to users, orgs, and other entities.
- [Customize Connect UI](/guides/auth/customize-connect-ui) - build your own authorization UI.
