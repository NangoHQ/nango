...
When you configure HTTP requests using Nango's helper ([reference](/reference/functions#http-requests)), you can specify the number of retries:
...
- Nango [sync functions](/guides/functions/sync-functions) can run for up to 24 hours, allowing retries to occur within this window (note that action functions and webhook functions have shorter lifespans; see [Resource limits](/guides/platform/limits))
- It works for APIs without requiring any API-specific configurations

Find details about HTTP request retries in the [reference](/reference/functions#http-request-retries).
...
- [Sync efficiency](/guides/functions/sync-efficiency) - avoid unnecessary provider requests.
...
- [Functions SDK reference](/reference/functions#http-request-retries) - configure request retries.
