...
```json
{
  ...
  "requests": [
    { "method": "GET", "endpoint": "/issues", "status": 200 }
  ],
  "diagnostics": null
}
```
...
- `requests` — the external API calls made during the run. Only populated when `--save-responses` is set; otherwise an empty array.
- `diagnostics` — memory and CPU stats. Only populated when `--diagnostics` is set; otherwise `null`.

On failure the `ok` field is `false` and `error` contains the error message (and `code` when available). Every failure path, including argument and configuration errors, still emits a single envelope, and the process exit code is preserved so CI can detect failure without parsing the body. This is designed for CI pipelines and agent harnesses that need to parse the dry run result.
