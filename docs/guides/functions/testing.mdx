...
# Machine-readable JSON output (useful for CI and agent tooling)
nango dryrun fetch-tickets abc-123 --json
```

When `--json` is set, the CLI suppresses human-readable formatting and writes a single JSON object to stdout:
...
```json
{
  ...
  "logs": {
    "counts": { "added": 5, "updated": 0, "deleted": 0 },
    "messages": [
      { "level": "info", "msg": "Fetched 5 tickets" }
    ]
  },
  "requests": [],
  "diagnostics": null
...
- `output` — for action functions, the returned value; for sync functions, the records that would have been persisted, keyed by model.
- `logs` — the log counts and messages the run produced.
- `requests` — the external API calls made during the run. Only populated when `--save-responses` (`--save`) is set; otherwise an empty array.
- `diagnostics` — memory and CPU stats. Only populated when `--diagnostics` is set; otherwise `null`.

On failure the `ok` field is `false` and `error` contains the error message (and `code` when available). Once the dry run starts, every failure path emits a single envelope and the process exit code is preserved, so CI can detect failure without parsing the body. Errors caught earlier by the CLI itself, such as missing arguments or invalid options, are still reported as plain text on stderr with a non-zero exit code. This is designed for CI pipelines and agent harnesses that need to parse the dry run result.
...
2. **Input data** is loaded from the `input` property for action functions.
...
4. The old mock directory  (`mocks` and `fixtures`) can then be safely deleted once a clean green test run is achieved.
...
<Warning>
**Pagination Bug in Legacy Test Utilities**

The legacy test utilities had a bug where pagination would sometimes stop after the first page. This means:

1. **Tests appeared to pass** but were only testing the first page of results
2. **Mock files may exist** for subsequent pages, but might be incorrect (hashes or params were never checked)
3. **Some mock files may be missing entirely** if they were never recorded

**How the migration handles this:**

The migration tool performs a **best-effort recovery**:
...
**What to do if tests fail after migration:**

If your tests fail after running the migration, it means one of:
...
**To fix failing tests, re-record your mocks:**
...
- [Data Validation](/guides/functions/data-validation) - Learn more about schema validation
- [Vitest Documentation](https://vitest.dev/) - Official Vitest docs
- [Action functions](/guides/functions/action-functions) - Building and testing on-demand functions
- [Sync functions](/guides/functions/syncs/sync-functions) - Building and testing sync functions
