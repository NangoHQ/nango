...
1. Your user authorizes an external API through [Nango Auth](/guides/auth/overview).
...
Template action functions can also be exposed as agent tools. Enable the action function, discover its schema with the [Get integration functions config API](/reference/api/scripts/config), then execute it through [tool calling](/guides/functions/tool-calling), the action trigger API, or Nango's MCP server.
...
<Steps>
  ...
  <Step title="Choose a function type">
    ...
    | --- | --- | --- | --- |
    | Keep external API data fresh in your app, or react to changes | [Sync function](/guides/functions/sync-functions) | Schedule, manual sync trigger, or external webhook reconciliation | Sync contacts/files/events/tickets, RAG, change detection / triggers |
  ...
  <Step title="Build the function">
    ...
    - [Sync functions](/guides/functions/sync-functions) — build scheduled data replication, checkpoints, metadata-driven parameters, and record consumption.
  ...
  <Step title="Test and deploy">
    ...
    For function-type-specific commands, see the [Sync function guide](/guides/functions/sync-functions#test-and-deploy), [Action function guide](/guides/functions/action-functions#test-and-deploy), and [Testing](/guides/functions/testing).
  </Step>
  ...
  <Step title="Receive webhooks from Nango">
    ...
    For sync webhooks, fetch changed records from the [records API](/reference/api/sync/records-list), process them in your app, and persist the last processed cursor. For forwarded external webhooks, see [Webhook forwarding](/guides/platform/webhook-forwarding).
