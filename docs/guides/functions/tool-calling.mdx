...
Tool calling in Nango happens through [action functions](/guides/functions/action-functions). Each tool is an action function that an agent, LLM SDK, or MCP client can call on demand for a specific connection.
...
Build custom tools with [action functions](/guides/functions/action-functions), or enable template action functions from the [template catalog](/guides/functions/functions-guide#template-catalog).
...
The [Get integration functions config API](/reference/backend/http-api/scripts/config) returns enabled action function definitions. Use it when an agent or orchestration layer should discover available tools at runtime.
...
Every request picks its connection and integration through headers, so the caller decides on each call. To fix the connections and the tool list up front and hand the agent a token instead of your API key, use [agent sessions](/guides/functions/agent-sessions).
...
See the [Auth guide](/guides/auth/auth-guide) and [Create Connect Session API](/reference/backend/http-api/connect/sessions/create) for the end-to-end auth flow.
...
<Accordion title="For agents">
  ...
  Required values for every execution are the Nango Environment API key, integration ID, connection ID, action function name, and input payload.
</Accordion>
...
- [Agent sessions](/guides/functions/agent-sessions)
- [Action functions](/guides/functions/action-functions)
- [Template catalog](/guides/functions/functions-guide#template-catalog)
- [Get integration functions config API](/reference/backend/http-api/scripts/config)
