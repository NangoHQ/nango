...
<Steps>
  ...
  <Step id="configure-api-keys" title="Configure API keys">
    ...
    <Accordion title="Team setup">
      Prefer a shared `dev` environment for day-to-day collaboration. Use [per-connection webhook URL overrides](/guides/platform/webhooks-from-nango#override-webhook-urls-per-connection) for local tunnels, and deploy functions granularly (`nango deploy --sync` / `--action` / `--integration`) so teammates don't overwrite each other. See [Engineering collaboration](/guides/platform/environments#engineering-collaboration) for the full pattern and tradeoffs of per-engineer environments.
      ...
      Use CI to keep the shared `dev` or `staging` baseline stable. See the [CI/CD guide](/guides/functions/ci-cd).
    </Accordion>
  ...
  <Step id="test-and-deploy" title="Test and deploy">
    ...
    Deploy the function you changed (preferred for local work on a shared environment):

    ```bash
    nango deploy --sync <function-name> dev
    ```

    A bare `nango deploy` reconciles the whole environment and can overwrite teammates' work — reserve it for CI or intentional full updates. See [Engineering collaboration](/guides/platform/environments#engineering-collaboration).
