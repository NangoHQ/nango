...
<Steps>
  ...
  <Step title="Configure API keys">
    Add your API keys to a `.env` file inside `nango-integrations/`:

    ```bash
    NANGO_API_KEY_PROD='<PROD-API-KEY>'
    NANGO_API_KEY_DEV='<DEV-API-KEY>'
    ```
    ...
    <Note>
      Keys are matched by environment name: `NANGO_API_KEY_<ENV_NAME>`. If you rename an environment in the UI, update the variable name in `.env` to match.
    </Note>
    ...
    <Accordion title="Team setup">
      ...
      ```bash
      NANGO_API_KEY_<YOUR_ENV_NAME>='<your-personal-dev-key>'
      ```
    ...
    <Accordion title="For agents">
      Run `nango init nango-integrations` to scaffold the folder. Ask the user for their API key from **Environment Settings > API Keys** and write it to `nango-integrations/.env`. Confirm that `nango-integrations/.gitignore` ignores `.env`.
