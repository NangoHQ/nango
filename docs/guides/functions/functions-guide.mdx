...
<Warning>
  Claude Code users: the default symlink install currently skips the `.claude/skills/` symlinks when `.claude/` doesn't already exist, so Claude Code won't see the skill. As a workaround, choose **Copy to all agents** during the interactive prompt (or create `.claude/` before running the command). Tracked upstream in [vercel-labs/skills#1138](https://github.com/vercel-labs/skills/issues/1138).
</Warning>
