...
3. Compile the source with `POST /remote-function/compile` and use compiler errors to improve it.
4. Dry run it with `POST /remote-function/dryrun` against a real connection and inspect the result, logs, records, or error.
5. Deploy it with `POST /remote-function/deploy`. After deployment, your app can call or trigger the function like any other Nango function.
