...
<Warning>
`deleteRecordsFromPreviousExecutions()` is incompatible with checkpoints because it requires comparing the full dataset between consecutive runs. Use `trackDeletesStart`/`trackDeletesEnd` instead — see [Deletion detection](/guides/functions/deletion-detection).
</Warning>

## Checkpoints vs. cursors

Both checkpoints and [cursors](/guides/functions/records-cache#cursors-and-sync-progress) are markers of sync progress, but they track different relationships:
