...
Nango is already multi-tenant by default — every sync function executes in the context of an end-user [Connection](/guides/auth/overview#overview), and runs independently for each connection. Variants are for the next level down: fan-out *within* one connection.
...
- [Sync functions](/guides/functions/sync-functions) - build the base sync before adding variants.
- [Checkpoints](/guides/functions/checkpoints) - track progress per variant.
- [Records cache](/guides/functions/records-cache) - fetch data for each variant.
- [Create sync variant API](/reference/api/sync/create-variant) - manage variants programmatically.
