...
1. Your user authorizes an external API through [Nango Auth](/guides/primitives/auth).
...
Template action functions can also be exposed as agent tools. Enable the action function, discover its schema with the [Get integration functions config API](/reference/api/scripts/config), then execute it through [tool calling](/implementation-guides/platform/functions/tool-calling), the action trigger API, or Nango's MCP server.
...
<Steps>
  ...
  <Step title="Choose a function type">
    ...
    | --- | --- | --- | --- |
    | Keep external API data fresh in your app, or react to changes | [Sync function](/implementation-guides/platform/functions/sync-functions) | Schedule, manual sync trigger, or external webhook reconciliation | Sync contacts/files/events/tickets, RAG, change detection / triggers |
    | Run an operation when your app or agent asks for it | [Action function](/implementation-guides/platform/functions/action-functions) | HTTP API, SDK, async action queue, or MCP/tool call | Create an issue, send a message, fetch details, perform a write |
    | Process an external provider webhook inside Nango | [Webhook function](/implementation-guides/platform/functions/webhook-functions) | External API sends a webhook to Nango | Update records from a webhook payload, normalize events, mark data for reconciliation |
    | React when a Nango connection is created, validated, or deleted | [Event function](/implementation-guides/platform/functions/event-functions) | Nango connection lifecycle event | Validate credentials, register provider webhooks, seed metadata, clean up subscriptions |

    <Tip>
      Use [webhook forwarding](/implementation-guides/platform/webhook-forwarding) instead of a webhook function when you want Nango to receive an external webhook, attribute it to a connection when possible, and forward the payload to your app for handling.
    </Tip>
  </Step>
  ...
  <Step title="Build the function">
    ...
    - [Sync functions](/implementation-guides/platform/functions/sync-functions) — build scheduled data replication, checkpoints, metadata-driven parameters, and record consumption.
    - [Action functions](/implementation-guides/platform/functions/action-functions) — build action function operations your app, jobs, or agents can call.
    - [Webhook functions](/implementation-guides/platform/functions/webhook-functions) — process external API webhooks inside Nango.
    - [Event functions](/implementation-guides/platform/functions/event-functions) — react to Nango connection lifecycle events.

    For shared primitives, use [Storage](/implementation-guides/platform/functions/storage) for connection metadata and [Data validation](/implementation-guides/platform/functions/data-validation) for input, output, and provider response validation.
  </Step>
  ...
  <Step title="Test and deploy">
    ...
    For function-type-specific commands, see the [Sync function guide](/implementation-guides/platform/functions/sync-functions#test-and-deploy), [Action function guide](/implementation-guides/platform/functions/action-functions#test-and-deploy), and [Testing](/implementation-guides/platform/functions/testing).
  </Step>
  ...
  <Step title="Receive webhooks from Nango">
    ...
    For sync webhooks, fetch changed records from the [records API](/reference/api/sync/records-list), process them in your app, and persist the last processed cursor. For forwarded external webhooks, see [Webhook forwarding](/implementation-guides/platform/webhook-forwarding).

    Read [Receive webhooks from Nango](/implementation-guides/platform/webhooks-from-nango) for signature verification, retry behavior, failure payloads, and the full webhook reference.
  </Step>
</Steps>
...
- [Function tool calling](/implementation-guides/platform/functions/tool-calling) — expose action functions to agents, LLM SDKs, and MCP clients.
- [Unified APIs](/implementation-guides/platform/functions/unified-apis) — build provider-specific functions behind one stable model.
- [Logging](/implementation-guides/platform/functions/logging) — control custom function logs locally and in cloud environments.
- [Storage](/implementation-guides/platform/functions/storage) — store per-connection metadata and read it inside functions.
- [Data validation](/implementation-guides/platform/functions/data-validation) — validate function input, output, and provider API responses.
- [Testing](/implementation-guides/platform/functions/testing) — dry runs, mocks, and snapshot tests.
- [Rate limits](/implementation-guides/platform/functions/rate-limits) — handle provider `429` responses and long-running retries.
- [CI/CD](/implementation-guides/platform/functions/ci-cd) — deploy functions with your application pipeline.
