...
| - | - |
| [Sync functions](/guides/functions/sync-functions) | 24 hours |
...
Each sync can have a maximum of **100 variants** per connection. For heavier workloads, reach out to increase this limit. See the [sync partitioning guide](/guides/functions/sync-partitioning) for more details.

### Action function output limits

Action functions have a maximum output payload size of **2MB**. For large payloads, use the [Proxy](/guides/platform/proxy/implement-requests-proxy) instead.
...
- [Sync efficiency](/guides/functions/sync-efficiency) - stay within payload and runtime limits.
