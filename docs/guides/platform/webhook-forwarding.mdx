...
Affinity, Fillout and ShipStation do not sign their webhooks, so Nango checks a secret you choose instead. Nango rejects every webhook from these providers until the secret is set.

1. Generate a random secret of at least 16 characters and save it as the **Webhook Secret** in **Integrations > [Integration] > Settings**.
2. Send the same secret with every webhook from the provider, either in the `X-Nango-Webhook-Secret` header or, when the provider only lets you set a URL, as a `nangoWebhookSecret` query param on the Nango webhook URL.
