...
Salesforce events come from an Apex trigger you install in each connected org, so send the `X-Nango-Webhook-Secret` header from the trigger's callout. The trigger is readable by that org's admins, so give each connection its own random secret of at least 16 characters by setting `webhookSecret` in its [connection metadata](/reference/backend/http-api/connections/set-metadata). Nango checks it before the integration secret.
