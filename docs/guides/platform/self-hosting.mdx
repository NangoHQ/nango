...
Connect UI is served at the root of its domain by default, but its prebuilt bundle uses relative asset paths and derives its routing base at runtime, so it works under any base path with no rebuild and no rewrite step. To host it under a non-root path — for example behind a reverse proxy that routes services by path — set `NANGO_PUBLIC_CONNECT_URL` to the full URL including the path:

```sh
NANGO_PUBLIC_CONNECT_URL=https://example.com/nango/connect
```

This URL drives the generated `connect_link`, the dashboard's Content Security Policy, and CORS, so it must match what end users load.
...
URLs loading Connect UI at a sub-path must end with a trailing slash (`…/connect/`) — the Nango SDK and the generated links produce this automatically. If the document is served at a slashless URL anyway, Connect UI recovers by retrying with a trailing slash; this requires the host to still serve `index.html` for the slashless URL (it cannot recover a URL the host refuses to serve).
