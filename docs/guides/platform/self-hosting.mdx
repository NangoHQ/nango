...
Nango rewrites the built asset paths to match when Connect UI starts, so no rebuild is needed. This works whether you serve Connect UI via `FLAG_SERVE_CONNECT_UI` or run its static file server directly (`npm run -w @nangohq/connect-ui serve:unsafe`, e.g. as a standalone container). If you need a base path that differs from the URL's path, set `NANGO_CONNECT_UI_BASE_PATH` to override it.
