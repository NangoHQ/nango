...
- Setting a cert without a key (or the reverse), setting both the inline and `_FILE` form of the same asset, or pointing `_FILE` at an unreadable path all fail at startup rather than silently falling back to plain TLS.
