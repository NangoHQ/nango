...
You can also subscribe to release notifications in GitHub by watching **Releases** on the private [`NangoHQ/managed-image-releases`](https://github.com/NangoHQ/managed-image-releases) repository (your GitHub user or team must have read access). Each release includes the Docker tag, version metadata, and a generated changelog between managed releases.
...
You can find the latest version in [`managed-manifest.json`](https://github.com/NangoHQ/managed-image-releases/blob/master/managed-manifest.json) on `managed-image-releases` (mirrored from [`NangoHQ/nango`](https://github.com/NangoHQ/nango/blob/master/managed-manifest.json) on each release).
