...
- Runners started at runtime by the jobs service inherit the configuration automatically, as inline PEM, regardless of which form you used on jobs itself. On Kubernetes that means the private key is visible in the runner pod spec. On AWS Lambda, all environment variables share a 4 KB budget that the assets can exhaust; prefer ECDSA and a single certificate rather than a chain.
