...
Nango rewrites the built asset paths to match on startup, so no rebuild is needed. If you need a base path that differs from the URL's path, set `NANGO_CONNECT_UI_BASE_PATH` to override it.
...
<Note>
If you serve Connect UI's static files from your own host instead of the bundled Docker image, run `npm run -w @nangohq/connect-ui set-base-path` (with `NANGO_PUBLIC_CONNECT_URL` set) against the built `dist` before uploading. The built assets contain a placeholder base until this step runs.
</Note>
