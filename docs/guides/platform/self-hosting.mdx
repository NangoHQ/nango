...
Do not put `NANGO_INTERNAL_AUTH_TOKEN` or `NANGO_INTERNAL_AUTH_SIGNING_KEY` on the runner service. Customer script code runs there; a fleet-wide jobs credential would be stealable after sandbox escape.
...
`putTask`/`heartbeat` use the invoke JWT.

**Zero-downtime flag flip:**

1. **Phase 0 — infra.** Inject the static token on server, jobs, and orchestrator, and the signing key on jobs. Current images ignore the extra env.
