...
Connect UI's prebuilt bundle uses relative asset paths and hash-based routing, so it works under any base path with no rebuild and no rewrite step. To host it under a non-root path — for example behind a reverse proxy that routes services by path — set `NANGO_PUBLIC_CONNECT_URL` to the full URL including the path:

```sh
NANGO_PUBLIC_CONNECT_URL=https://example.com/nango/connect
```

This URL drives the connect link Nango generates as well as the server's CSP and CORS configuration, so it must match what end users load in the browser.
...
Direct loads should use a trailing slash (`https://example.com/nango/connect/`) so relative assets resolve — the Nango SDK, generated connect links, and an in-page guard handle this automatically.
