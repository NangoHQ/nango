...
End users must load Connect UI with a trailing slash (`https://example.com/nango/connect/`). Assets resolve relative to the document URL, so the slashless form loads a blank page. Links Nango generates already carry it; if end users can reach the slashless form directly, redirect it to the trailing-slash form in your reverse proxy.
