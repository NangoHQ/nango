...
- **Control plane** (server, jobs, orchestrator): a static shared secret, or on Kubernetes a projected ServiceAccount token. Used for server/jobs calling orchestrator, and for runner `register`/`idle`.
- **Task JWT**: jobs mints an HMAC token at invoke time (`iss: nango-internal`, `aud: jobs`, `task_id`) and passes it in the runner `start` call or Lambda payload. Runners present it on `putTask`/`heartbeat`. The signing key never leaves jobs. A stolen task token can at most forge **that** task. The default lifetime is seven days so scheduled syncs (orchestrator started-to-completed is one day) can finish without a refresh; Lambda uses the task kill deadline plus a buffer.

Do not put `NANGO_INTERNAL_AUTH_TOKEN` or `NANGO_INTERNAL_AUTH_SIGNING_KEY` on runners or Lambda. Customer script code runs there; a fleet-wide jobs credential would be stealable after sandbox escape.

**ECS, docker-compose, and local:**

```sh
# server, jobs, and orchestrator only
NANGO_INTERNAL_AUTH_TOKEN=<SHARED-BEARER>
# jobs only
NANGO_INTERNAL_AUTH_SIGNING_KEY=<HMAC-SECRET>
```

No ServiceAccounts, projected volumes, or TokenReview. Runner tasks optionally get the static token **only if** they call `register`/`idle`. `putTask`/`heartbeat` use the invoke JWT.

**Kubernetes (Helm follow-up in `nango-helm-charts`):**

Projected ServiceAccount tokens replace the control-plane static secret. Charts should provide:

- ServiceAccounts `nango-server`, `nango-jobs`, `nango-orchestrator`, and `nango-runner` (create per namespace if `NAMESPACE_PER_RUNNER`)
- Projected volumes: server and jobs with `audience: orchestrator`; runner pods with `audience: jobs` for register/idle only (`automountServiceAccountToken: false`)
- A TokenReview ClusterRole (`tokenreviews/create`) bound to the orchestrator and jobs ServiceAccounts
- Optional `create` on `serviceaccounts` in the jobs role when `NAMESPACE_PER_RUNNER` is enabled, so jobs can create `nango-runner` in namespaces it just created. A 403 on that POST does not abort: Kubernetes authorizes `create` before AlreadyExists, so jobs then adopts the Helm-provisioned account. Grant `create` only when Helm or a policy does not provision `nango-runner` in those namespaces.
- `NANGO_INTERNAL_AUTH_SIGNING_KEY` as a secret on jobs only

Set `NANGO_INTERNAL_AUTH_RUNNER_SERVICE_ACCOUNT=nango-runner` on jobs so it projects the runner volume. Leave it unset until the ServiceAccount exists, or new runner pods fail to start.

**Zero-downtime flag flip:**

1. **Phase 0 — infra.** Inject the static token on server/jobs/orchestrator and the signing key on jobs. On Kubernetes, apply projected volumes and TokenReview RBAC. Current images ignore the extra env.
2. **Phase 1 — app image, `NANGO_INTERNAL_AUTH_REQUIRED` unset or `false`.** Roll in any order. Callers send a credential when they have one; servers verify if present and accept if absent. Jobs starts minting task JWTs. Drain in-flight tasks that started before this image (they heartbeat without a JWT).
3. **Phase 2a — orchestrator inbound.** Set `NANGO_INTERNAL_AUTH_REQUIRED=true` on orchestrator once every server and jobs replica is on phase 1. Closes open dequeue. Does not need the runner drain.
4. **Phase 2b — jobs inbound.** Set `NANGO_INTERNAL_AUTH_REQUIRED=true` on jobs after the drain. Same jobs image; this is not a second deploy. Unauthenticated `putTask`/`heartbeat` then return 401.

Do not default `NANGO_INTERNAL_AUTH_REQUIRED` to true in the same release that introduces minting. Do not set it on jobs before the drain: in-flight heartbeats from old tasks would 401 and fail customer syncs.

| Variable | Where | Purpose |
| - | - | - |
| `NANGO_INTERNAL_AUTH_TOKEN` | server, jobs, orchestrator | Static Bearer fallback. Never runners or Lambda. |
| `NANGO_INTERNAL_AUTH_TOKEN_FILE` | control plane or K8s runners | Path to a projected SA token. Missing file is treated as no credential. |
...
| `NANGO_INTERNAL_AUTH_RUNNER_SERVICE_ACCOUNT` | jobs (Kubernetes) | When set, project a jobs-audience SA token onto runner pods for register/idle. |
