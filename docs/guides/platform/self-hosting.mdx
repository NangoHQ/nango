...
Connect UI is a single-page app: the server must respond with `index.html` for any path that isn't a built asset. Nango's static file server handles this out of the box; if you serve the built `dist` from your own static host, enable its SPA fallback.

Connect UI loads best with a trailing slash (`https://example.com/nango/connect/`), because assets resolve relative to the document URL. Links Nango generates already carry it. The slashless form still works — the page detects the failed asset load and redirects once — but it costs a round trip, so redirect it in your reverse proxy if end users can reach it directly.
