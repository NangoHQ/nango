...
Enable it on the server with two variables:

```bash
FLAG_AUDIT_TRAIL_ENABLED=true
AUDIT_DATABASE_URL=postgres://user:password@host:5432/nango
```

Both are required: without a database there is nowhere to keep entries, so the trail stays off rather than showing a page that never fills. The URL can point at the database Nango already uses or at a separate one, and the server creates the `audit` schema and its table on startup, along with the rest of the schema, when `NANGO_MIGRATE_AT_START=true`.

Entries live in a single table partitioned by day. The server creates the next day's partition ahead of time and drops whole partitions once they fall outside the retention window, so expiring old entries never scans the table:

```bash
NANGO_AUDIT_POSTGRES_RETENTION_DAYS=365
```

Two optional settings control the connection:

| Variable | Default | Description |
|----------|---------|-------------|
| `AUDIT_DB_POOL_MAX` | `5` | Connections in the audit pool. The trail writes one row per control-plane request, so the default suits most instances. |
| `AUDIT_DB_SSL` | `false` | Encrypt the connection. An `sslmode` in `AUDIT_DATABASE_URL` takes precedence, and is how you require certificate verification or point at a private CA with `sslrootcert`. |

<Note>
These variables configure a Postgres-backed trail and apply to self-hosted instances only. They are unrelated to the `audit-trail` feature flag below, which Nango Cloud uses to roll the feature out per account.
</Note>
