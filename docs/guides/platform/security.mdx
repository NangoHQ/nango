...
| Data | Includes | Encryption |
| - | - | - |
| Connection credentials | OAuth access and refresh tokens, API keys, other provider credentials | **AES-256-GCM** |
| Integration credentials | Your OAuth client secrets and custom provider settings | **AES-256-GCM** |
| Nango API keys | Account and environment API keys | **AES-256-GCM** |
| Environment variables | Values you set per environment | **AES-256-GCM** |
| Synced records | Record payloads written to the [records cache](/guides/functions/syncs/records-cache) by sync functions | **AES-256-GCM** |
| Connection configuration | OAuth scopes, authorization parameters, provider-specific settings | Storage-level |
| Logs & audit trail | Operational and control-plane activity | Storage-level |

Every row is encrypted at rest at the storage layer. Rows marked **AES-256-GCM** carry an additional application-level layer: the value is encrypted before it is written, so it stays unreadable to anyone holding database access alone.

Data is stored in a managed PostgreSQL database in AWS, network-isolated in a private VPC, with automated backups and point-in-time recovery. Self-hosted deployments run on infrastructure you control.
...
| Data | Retained | Deletion |
| - | - | - |
| Connections & metadata | Until you delete the connection | Soft delete immediate, hard delete after **31 days** |
| [Synced records](#synced-records-retention) | Payload **30 days** past last update, record **60 days** past last sync run | Automatic, or with the connection |
| [Nango logs](#nango-logs) | **15 days** on Nango Cloud | Automatic at end of retention; no manual deletion |
| [Audit trail](#audit-trail) | **1 year** by default | Immutable, never deleted before retention ends |

All retention periods are configurable when [self-hosting](/guides/platform/self-hosting).

### Deleting a connection

Delete from the dashboard, the [delete connection endpoint](/reference/backend/http-api/connections/delete), or `deleteConnection` in any backend SDK. Deletion is two-stage:

1. **Soft delete (immediate)** — the connection becomes inaccessible, its syncs stop, and scheduled tasks are cancelled.
2. **Hard delete (31 days later)** — credentials, metadata, configuration, synced records, and job history are permanently removed. The window allows recovery from accidental deletion.
...
<Tip>
**Further questions?** [Contact us](https://nango.dev/contact) and we are happy to help.
</Tip>

## Security disclosure

Found a vulnerability in Nango? Please review our [security policy](https://nango.dev/security) and follow it to submit your findings. We appreciate responsible disclosures!
