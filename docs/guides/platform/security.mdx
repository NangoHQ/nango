...
For your own reviews, the [audit trail](/guides/platform/audit-trail) records who changed what in your account, when, and from where.

Visit our [Trust Center](https://trust.nango.dev) for the latest compliance documentation, security policies, and audit reports.
...
2. **Associated data cleanup**: Sync functions are stopped, and scheduled tasks are cancelled
...
Set up [webhooks from Nango](/guides/platform/webhooks-from-nango) to be notified when new data is available. Fetch the updated records immediately after receiving a sync webhook.
...
- **API**: Call the [delete connection endpoint](/reference/backend/http-api/connections/delete)
...
| - | - | - |
| Environment API key | Server-to-server API access | Long-lived (until rotated) |
...
- Environment API keys are environment-specific (dev, prod, etc.)
- Keys can be rotated via the dashboard
- All API requests require a valid Environment API key in the `Authorization` header
...
The [audit trail](/guides/platform/audit-trail) records control-plane changes with the actor, the outcome, and the originating IP: who created or deleted a connection, changed an integration, rotated an API key, changed a teammate's role, or signed in. Entries are immutable and account-wide. It is available to Enterprise customers by default; other paid plans can contact us to have it enabled.

<Warning>
**Credential reads are not recorded.** Retrieving a connection's credentials produces no audit entry, and neither does data-plane traffic such as proxy requests, action executions, or record reads. See [what is not recorded](/guides/platform/audit-trail#what-is-not-recorded).
</Warning>

### Activity logs

[Activity logs](/guides/platform/observability) are a separate, operational record of what ran — auth flows, function executions, proxy requests — and can be exported via [OpenTelemetry](/guides/platform/observability#opentelemetry-export).

They answer "what happened in my integrations", where the audit trail answers "who changed my account". Neither records the credential reads described above.
...
Whether an environment is considered production is configured in **Environment Settings → General**. The production status of the current environment is also visible in the environment dropdown at the top left of the dashboard.

The flag can be set on any environment regardless of its name — not just `prod`. This lets you protect environments like `prod-eu`, `production`, or `live` the same way.
...
<Info>
These roles apply to dashboard access only. They do not affect API authentication — any caller with a valid API key retains full API access regardless of their dashboard role.
</Info>

<a id="sso"></a>
## Single sign-on (SSO)

Nango Cloud uses [WorkOS](https://workos.com) to power SSO.
...
Once SSO is configured, users sign in through your company's identity provider (IdP). Nango currently does not provide a dedicated SSO button on the login page — login must be initiated from the IdP side.
...
For details on available scopes, advised profiles, and key management, see the [API Keys reference](/reference/backend/http-api/api-keys).
