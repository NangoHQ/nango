...
Data is stored in a managed PostgreSQL database in AWS, network-isolated in a private VPC, with automated backups and point-in-time recovery. Self-hosted deployments run on infrastructure you control, and all retention periods are configurable when [self-hosting](/guides/platform/self-hosting).
