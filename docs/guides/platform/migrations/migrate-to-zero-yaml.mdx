...
<div className="agent-quickstart-actions">
  <button
    ...
    data-copy-text="npx skills add NangoHQ/skills -s migrating-to-zero-yaml"
    data-copy-success="Install command copied"
  >
    <span className="agent-quickstart-action-title">Install Zero YAML migration skill</span>
    <span className="agent-quickstart-action-description">
      Copy the command to install <code>migrating-to-zero-yaml</code> from <code>NangoHQ/skills</code>.
    </span>
  </button>
</div>

The `nango.yaml` configuration file is no longer supported. Since September 15, 2026, Nango rejects deployments that use it. This guide helps you migrate your existing integrations from the YAML-based approach to Zero YAML, which uses pure TypeScript with modern tooling.
...
**Q: Is nango.yaml still supported?**

**A:** No. Support ended on September 15, 2026. Deployments that use a `nango.yaml` are rejected with an error that points to this guide. Functions deployed before then keep running until you redeploy them, which requires migrating first.
...
**A:** Not to `nango.yaml`, since deployments that use it are rejected. Validate the migrated integration before it reaches production: run `nango dryrun` against a test connection, then deploy to a development environment with `nango deploy dev`. Keep a copy of your original files as a reference until you have validated the migration.
