...
The [Control Plane MCP server](/reference/backend/control-plane-mcp/logs) provides Beta tools for listing and filtering log operations and retrieving their messages from an MCP-compatible client. Authenticate with a Nango API key that has the `environment:logs:read` scope.
