...
The [Management MCP server](/reference/backend/management-mcp/overview#logs) provides Beta tools for listing and filtering log operations and retrieving their messages from an MCP-compatible client. Authenticate with a Nango API key that has the `environment:logs:read` scope.
