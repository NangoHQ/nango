---
title: "Audit trail"
sidebarTitle: "Audit trail"
description: "Review who changed what in your Nango account."
---

The audit trail records **control-plane** activity in your Nango account — the operations that configure it and manage who can reach it: connection changes, integration settings, teammate roles, sign-ins.

Each entry captures the actor, the action, the affected resource, and the outcome, so you can answer "who did this, and when" during an incident or a compliance review.

Entries are immutable: Nango never edits or deletes one before its retention period ends.

## Availability

The audit trail is available on the [Enterprise plan](https://nango.dev/pricing), with a default 1 year data retention. [Contact us](https://nango.dev/demo) if you would like a demo.

## Viewing & exporting Audit trail entries

When enabled for your account, the audit trail lives in the Nango dashboard under **Audit trail**, in the menu behind your profile. It covers all activity in your account, across all environments. Narrow it by time window, resource and action, and open any entry to see its full detail.

### Exporting to CSV

**Export** downloads entries as a CSV. The time window and the resource and action filters selected on the page are what gets exported, so narrow them first if you want a subset.

A single export returns at most 50,000 entries, most recent first. If your selection holds more, the download still succeeds and the dashboard tells you it was truncated — narrow the window, or [contact us](https://nango.dev/demo) for a larger or scheduled export.

The file has one row per entry, with the columns `occurred_at`, `event_id`, `resource`, `action`, `outcome`, `actor_type`, `actor_id`, `actor_display`, `via`, `environment`, `targets`, `ip`, `user_agent`, `interface` and `metadata`.

The dashboard and this export are the two ways to get at the trail today — there is no API for reading it. [Talk to us](https://nango.dev/demo) if you need to feed it into a SIEM.

<a id="what-is-recorded"></a>
## What is recorded

Each entry records:

| Field | Description |
|-------|-------------|
| `id` | A unique ID for the entry, useful when referring to one in a support request |
| `occurredAt` | When the action happened |
| `actor` | Who performed it — see [Actors](#actors) |
| `via` | Only present when the action reached Nango through a Nango support session — see [Nango support access](#support-access) |
| `resource` and `action` | What was done, for example `connection` + `deleted` |
| `targets` | What it was done to, such as a connection ID or a teammate's email |
| `outcome` | `success`, `failure`, or `denied` |
| `environment` | The environment involved, or empty for account-level actions |
| `context` | The originating IP address and user agent, and the interface used: `api` covers both the dashboard and the REST API, `mcp` covers Nango's management MCP server |
| `metadata` | Detail specific to the action, such as whether a sync run was a full refresh, or the filters an export used |

The audit trail covers the main control-plane operations related to connections, integrations, functions, syncs, API keys, environments, team membership, authentication, and billing. The recorded `resource`s and `action`s are:

| Resource | Actions |
|----------|---------|
| `connection` | `created`, `updated`, `metadata_updated`, `refreshed`, `deleted` |
| `integration` | `created`, `updated`, `deleted` |
| `function` | `deployed`, `upgraded`, `deleted` |
| `sync` | `enabled`, `disabled`, `paused`, `started`, `triggered`, `cancelled`, `frequency_changed`, `variant_created`, `variant_deleted` |
| `api_key` | `created`, `updated`, `deleted` |
| `environment` | `created`, `updated`, `variables_changed`, `webhook_urls_changed`, `webhook_signing_key_rotated`, `deleted` |
| `member` | `invited`, `invite_accepted`, `invite_declined`, `invite_revoked`, `removed`, `role_changed` |
| `team` | `updated` |
| `user` | `updated` |
| `app_auth` | `login`, `logout`, `signup`, `password_changed`, `password_reset` |
| `mfa` | `enrolled`, `enabled`, `disabled`, `verified`, `recovery_regenerated` |
| `billing` | `plan_changed`, `trial_extended`, `details_changed`, `payment_method_added`, `payment_method_removed` |
| `audit_trail` | `queried`, `exported` |

Secrets, such as API keys and passwords, are stripped from audit log entries.

Reads are generally not recorded. Listing connections, fetching an integration, or opening a page in the dashboard leaves no entry. Our audit trail is designed to answer "what changed", not "what was looked at".

The audit trail itself is the exception: reading it and exporting it are both recorded, as `audit_trail.queried` and `audit_trail.exported`, along with the filters used. Who looked at an audit trail is itself worth auditing, so the trail is held to a stricter standard than the rest of the platform.

<Note>
The audit trail currently does not cover data plane operations, such as reading connection credentials, fetching sync records, or triggering a sync over the public API. See [Limitations](#limitations) for details.
</Note>

<a id="actors"></a>
## Actors

| Actor | Meaning |
|-------|---------|
| `user` | Someone signed in to the Nango dashboard |
| `api_key` | A call authenticated with an API key. The entry includes the API key's name. |
| `connect_session` | The end user who completed a Connect flow, identified by the end-user ID you supplied when creating the session, and their email if you supplied one |
| `anonymous` | An action where the caller can't be known. For example, a rejected sign-in |
| `unknown` | A caller Nango could not attribute |

Calls your functions make — a sync or an action running on Nango — are also recorded as `api_key`, because they authenticate with the environment's secret key. They are attributed to the environment rather than to the function that made them, so an entry cannot tell a function's write apart from a person using that same key. [Let us know](https://nango.dev/demo) if you need function-level attribution.

<a id="support-access"></a>
## Nango support access

Nango staff may access your account to investigate a support request. Actions taken during such a session carry `via`, naming the Nango account that was acting, next to the `actor` the action was performed as. An entry without `via` was not reached through a Nango session.

The marker covers the actions taken during a session. The start and end of the session are not themselves entries today.

<a id="limitations"></a>
## Limitations

<Note>
We can expand audit trail coverage on request. If you need items logged that aren't covered today, please [reach out](https://nango.dev/demo)!
</Note>

The audit trail currently covers the control plane, but not the **data plane**: API requests used to work with integrations and exchange data with external APIs.

For example, the following activities are not covered:

| Not recorded | Example |
|--------------|----------|
| Reading connection credentials | `GET /connection/{connectionId}` |
| Reading records from Nango | `GET /records` |
| Deleting/pruning records in Nango | `PATCH /records` pruning |
| Proxy requests | Every call through `/proxy` |
| Action executions | `POST /action/trigger` |
| Sync triggers over the public API | `POST /sync/trigger` — runs started from the dashboard *are* recorded, as `sync` + `triggered` |
| Reading configuration | Listing or fetching connections, integrations, or functions |