...
| `public_key` | A connection created through the [legacy public key flow](/guides/platform/migrations/migrate-from-public-key). The key identifies your environment, not a person, so no end user is named |
| `unknown` | Nango could not attribute the caller. In practice this is a connection completed by the provider rather than by anyone on your side, such as a GitHub App installation callback |
