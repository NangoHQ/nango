...
| `context` | The originating IP address and user agent, and which interface was used (dashboard, api, etc.) |
...
<Note>
The audit trail currently does not cover data plane operations, such as reading connection credentials, fetching sync records, or sync triggers. See [Limitations](#limitations) for details.
</Note>
...
| `api_key` | A call authenticated with an API key. The entry includes the API key name. |
