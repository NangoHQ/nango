...
A single connection can override the environment's webhook URLs with its own URL. The connection still receives all the same [webhook types](#types-of-nango-webhooks), with the same signing, retries, and logs.

When this override is set, the connection's webhooks are sent **only** to that URL — the environment's primary and secondary webhook URLs are both bypassed for that connection. It does not add a third destination.

Set `webhook_url_override` when creating a [connect session](/reference/backend/http-api/connect/sessions/create). It applies to the connection created by that session:
...
<Warning>
Verify the signature with the **webhook signing key** from **Environment Settings > Webhooks > Signing key** — not your Environment API key. These are two distinct keys. With the Node SDK, pass it as `webhookSigningKey`; for raw HMAC verification, use it as the HMAC secret. Using the Environment API key will cause verification to fail on any environment where the keys differ.
</Warning>
...
<Tabs>
  ...
  <Tab title="JavaScript/TypeScript">
    ...
    const hash = crypto.createHmac('sha256', webhookSigningKey).update(body).digest('hex');
    ```
  </Tab>
  <Tab title="Python">
    ```python
    ...
    hash = hmac.new(webhook_signing_key.encode('utf-8'), body, hashlib.sha256).hexdigest()
    ```
  </Tab>
  <Tab title="Java">
    ```java
  ...
  <Tab title="Rust">
    ...
    let mut mac = HmacSha256::new_from_slice(webhook_signing_key.as_bytes()).unwrap();
  ...
  <Tab title="PHP">
    ...
    $hash = hash_hmac('sha256', $body, $webhookSigningKey);
    ?>
    ```
  </Tab>
</Tabs>
...
<Note>
  The `override` webhook has its own webhook setting in _Environment Settings_, separate from the
  other auth webhooks.
</Note>
