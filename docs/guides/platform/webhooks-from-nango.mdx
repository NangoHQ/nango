...
Sync webhooks are sent when a [sync function](/guides/functions/sync-functions) execution finishes, whether successful or not.
...
`modifiedAfter` is an ISO-8601 timestamp marking the start of the last run. To read the changed records, use cursor-based fetching as described in [Records cache → Cursors and sync progress](/guides/functions/records-cache#cursors-and-sync-progress).
