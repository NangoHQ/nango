...
<Tabs>
  <Tab title="Node">
    ```ts
    // Optional: set NANGO_CONNECTION_WEBHOOK_URL locally to your tunnel URL.
    // Leave unset in shared/staging/prod so environment webhook URLs apply.
    const webhookUrl = process.env.NANGO_CONNECTION_WEBHOOK_URL;
...
<Tip>
    **Local development:** set `NANGO_CONNECTION_WEBHOOK_URL` to your tunnel (e.g. [ngrok](https://ngrok.com/) or [webhook.site](https://webhook.site/)) so new connections route webhooks to your machine — without changing the shared environment webhook URLs. See [Engineering collaboration](/guides/platform/environments#engineering-collaboration) for the full multiplayer pattern.
</Tip>
