...
description: 'Reference for the webhooks Nango sends to your app — setup, types, payloads, signature verification, and retries.'
---
...
<Info>
To process webhooks coming **from external APIs** (not from Nango), see [Process external webhooks](/implementation-guides/use-cases/webhooks-from-external-apis). To forward external API webhooks through Nango to your app, see [Webhook forwarding](/implementation-guides/platform/webhook-forwarding).
</Info>
...
Sync webhooks are sent when a [sync function](/implementation-guides/platform/functions/sync-functions) execution finishes, whether successful or not.
...
`modifiedAfter` is an ISO-8601 timestamp marking the start of the last run. To read the changed records, use cursor-based fetching as described in [Records cache → Cursors and sync progress](/implementation-guides/platform/functions/records-cache#cursors-and-sync-progress).
...
Forwarded external webhooks use the same webhook URLs, signing, retries, and logs as other Nango webhooks. See [Webhook forwarding](/implementation-guides/platform/webhook-forwarding) for setup, routing behavior, and payload shapes.
...
- [Webhook forwarding](/implementation-guides/platform/webhook-forwarding) - forward external provider webhooks through Nango.
- [Webhook functions](/implementation-guides/platform/functions/webhook-functions) - process provider webhooks inside Nango.
- [Event functions](/implementation-guides/platform/functions/event-functions) - react to connection lifecycle events.
