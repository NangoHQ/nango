...
<Info>
To process webhooks coming **from external APIs** (not from Nango), see [Process external webhooks](/implementation-guides/use-cases/webhooks-from-external-apis).
</Info>
...
Sync webhooks are sent when a [sync](/implementation-guides/use-cases/syncs) execution finishes, whether successful or not.
...
`modifiedAfter` is an ISO-8601 timestamp marking the start of the last run. To read the changed records, use cursor-based fetching as described in [Records cache → Cursors and sync progress](/implementation-guides/use-cases/syncs/records-cache#cursors-and-sync-progress).
