...
MCP Auth lets your application authorize end users to access external MCP servers, such as Notion MCP, Linear MCP, or HubSpot MCP, so you can call those servers’ tools on behalf of each user. Nango handles OAuth with the MCP provider and stores tokens; you then use the connection to call the MCP server via Nango’s [proxy](/reference/api/proxy/post).
...
| **MCP Server OAuth2 (Generic)** | Any MCP-compatible server that supports OAuth 2.0 with [RFC 8414](https://tools.ietf.org/html/rfc8414). | Custom or third-party MCP servers |
...
3. **Call the MCP server**  
   Use the stored `connection_id` and integration id (e.g. `hubspot-mcp`) with the [Proxy API](/reference/api/proxy/post) (JSON-RPC `tools/call`, etc.).
...
4. **Call the MCP server**  
   Same as pre-built: use the connection with the [Proxy API](/reference/api/proxy/post). The connection is tied to the MCP server URL the user entered.
...
- **MCP Generic**  
  User clicks connect → sees a form asking for **MCP Server URL** → submits → is redirected to that MCP server’s OAuth page → signs in and approves → redirected back; connection is created.
...
- **Credentials**  
  ...
  - **Static registration** (e.g. HubSpot MCP): you must register an MCP auth app with the provider and add **Client ID** and **Client Secret** in the integration settings. See each integration’s setup guide (e.g. [HubSpot MCP](/api-integrations/hubspot-mcp/how-to-register-your-own-hubspot-mcp-api-oauth-app)).
