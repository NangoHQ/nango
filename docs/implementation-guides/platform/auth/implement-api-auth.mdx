...
<Accordion title="ℹ️ Tags">
  `tags` are copied onto the connection, included in auth webhooks, and can be used to filter connections.

  Most apps start with: `end_user_id`, `end_user_email`, `organization_id`.

  For more details, see [Tagging a new connection](/implementation-guides/platform/auth/tagging-new-connection).
</Accordion>
