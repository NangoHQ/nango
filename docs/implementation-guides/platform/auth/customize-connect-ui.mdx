...
<Info>
Pre-requisites:
- complete the [Set up an integration](/guides/primitives/auth#guide) section of the Auth guide
- generate a [Connect session token](/guides/primitives/auth#guide)
</Info>
...
Initiate the authorization flow ([reference](/reference/sdks/frontend#connect-using-the-headless-client)):
...
When using the [Connect UI](/guides/primitives/auth#guide), this information is collected from the end user with an automatically-generated form.

But when using a custom UI, you must provide this configuration when calling `nango.auth()` ([reference](/reference/sdks/frontend#connect-using-the-headless-client)):
...
This _connection configuration_ is stored in the connection. You can retrieve it with the SDK ([reference](/reference/sdks/node#get-a-connection-with-credentials)) and API ([reference](/reference/api/connections/get)), or see it in connection details in the Nango UI.

## Override documentation links

By default, the Connect UI displays info icons that link to the Nango documentation. If you are embedding Connect in a customer-facing application and want these links to point to your own documentation instead, you can override them per integration using the `overrides` parameter when [creating a Connect session](/reference/api/connect/sessions/create).
...
<Tab title="Node SDK">
See the full [SDK reference](/reference/sdks/node#create-a-connect-session) for all available parameters.
...
<Tab title="REST API">
See the full [API reference](/reference/api/connect/sessions/create) for all available parameters.
...
# Next
Once the authorization succeeds, [_save the connection ID_](/guides/primitives/auth#guide).
...
- [Auth guide](/guides/primitives/auth) - compare the managed Connect UI flow.
- [Frontend SDK reference](/reference/sdks/frontend#connect-using-the-headless-client) - headless authorization methods and parameters.
- [Connection tags](/implementation-guides/platform/auth/connection-tags) - attribute custom UI flows to your own users and orgs.
