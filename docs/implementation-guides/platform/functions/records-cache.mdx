...
Every time you call [`nango.batchSave()`](/reference/functions#save-records) or [`nango.batchDelete()`](/reference/functions#delete-records) inside a sync function, you are writing to this cache. Your application then reads from the cache using the [GET /records](/reference/api/sync/records-list) endpoint or the [Node SDK](/reference/sdks/node#get-records).
...
See [Re-syncing: resetting checkpoints and the cache](/implementation-guides/platform/functions/checkpoints#re-syncing-resetting-checkpoints-and-the-cache) in the Checkpoints guide for the `reset` / `emptyCache` options and their implications.
...
- [Sync functions](/implementation-guides/platform/functions/sync-functions) - write records into the cache.
- [Sync efficiency](/implementation-guides/platform/functions/sync-efficiency) - keep records small and consume changes promptly.
- [Deletion detection](/implementation-guides/platform/functions/deletion-detection) - mark records deleted in the cache.
- [Get records API](/reference/api/sync/records-list) - fetch changed records from your app.
