...
<Warning>
  The output of an action function cannot exceed 2 MB. Return only the fields your app needs. For larger payloads, use the [requests proxy](/implementation-guides/platform/proxy/implement-requests-proxy) to fetch data directly from your app.
</Warning>
...
For broader test coverage, see the [testing guide](/implementation-guides/platform/functions/testing). To deploy from CI, see [CI/CD](/implementation-guides/platform/functions/ci-cd).
...
To receive completion events instead of polling, configure [webhooks from Nango to your app](/implementation-guides/platform/webhooks-from-nango) and enable the `Async action completed` webhook.
...
- [Function tool calling](/implementation-guides/platform/functions/tool-calling)
- [Data validation](/implementation-guides/platform/functions/data-validation)
- [Rate limits](/implementation-guides/platform/functions/rate-limits)
