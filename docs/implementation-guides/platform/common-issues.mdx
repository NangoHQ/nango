...
- The only solution is to ask the user to re-authenticate. See: [How to trigger a re-authorization flow in Nango](/guides/primitives/auth#re-authorize-an-existing-connection)
...
- [Auth guide](/guides/primitives/auth#re-authorize-an-existing-connection) - repair broken connections with re-authorization.
- [Token refreshing and validity](/implementation-guides/platform/auth/token-refreshing) - understand refresh failures.
- [Webhooks from Nango](/implementation-guides/platform/webhooks-from-nango) - receive failure notifications.
