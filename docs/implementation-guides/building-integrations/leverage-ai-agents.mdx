...
Install the integration builder for your preferred AI tool:
...
### Using the Builder (Claude Code)

If you're using [Claude Code](https://docs.anthropic.com/en/docs/claude-code), start a session and run the `/build-actions` slash command:
...
Once you provide your input, the Claude agent automatically:
...
### Using the Builder (Codex)

If you're using [Codex](https://github.com/openai/codex), start a session and run the `build-actions` command (without a slash, as Codex doesn't support slash commands):
...
The builder will enter **Interactive Mode** with the same prompts as Claude Code (see table above).
