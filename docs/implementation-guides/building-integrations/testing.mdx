...
Use the `--validate` (or `--validation`) flag to enforce validation:
...
```
Invalid sync record. Use `--validation` option to see the details (invalid_sync_record)
{
  "validation": [
    {
      "path": "createdAt",
...
Use the `--save` (or `--save-responses`) flag to save all API responses:
...
### 1. Generate fresh mocks regularly

API responses change over time. Regenerate mocks periodically:

```bash
# Regenerate mocks for all integrations
nango dryrun fetch-tickets connection-1 --save
nango dryrun create-ticket connection-1 --save --input @fixtures/test-input.json
```

### 2. Use realistic test data

When testing actions, use realistic input data that represents actual use cases:

```json
// fixtures/create-ticket-input.json
{
...
```typescript
// ✅ Good - focused test
it('should normalize priority values to lowercase', async () => {
  // Test one thing
});

// ❌ Bad - testing multiple things
it('should transform data correctly', async () => {
  // Testing priority, dates, IDs, filtering, etc.
});
```
...
```typescript
// ✅ Good
it('should convert Unix timestamps to ISO 8601 format', async () => { });
...
Commit mock files to version control so tests are reproducible across environments:

```bash
git add github/mocks/
git commit -m "Update test mocks for GitHub integration"
```

## Troubleshooting

### Tests fail after regenerating mocks

If tests fail after running `--save` again, your integration logic may have changed. Review the differences:

```bash
git diff github/mocks/fetch-tickets/Ticket/batchSave.json
```

Update your tests or fix your integration logic accordingly.

### Mock files not loading

Ensure your mock directory structure matches the expected format:
- Directory name matches `dirname` in test
- Sync/action name matches `name` in test
- Model names match exactly (case-sensitive)

### Validation errors in dry run

If validation fails during dry run:

1. Check the error output for specific field issues
2. Verify your model definitions in your integration
3. Ensure API responses match your expected schema
4. Update your data transformation logic to fix validation errors

### Tests work locally but fail in CI

Ensure mock files are committed to version control:

```bash
git status github/mocks/
```

If mocks are gitignored, update your `.gitignore` to include them.
...
- [Actions](/implementation-guides/actions/overview) - Building and testing actions
- [Syncs](/implementation-guides/syncs/overview) - Building and testing syncs
