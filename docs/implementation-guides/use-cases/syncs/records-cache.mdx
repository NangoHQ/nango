...
When you [trigger a sync run](/reference/api/sync/trigger) manually (from the UI or API), you have three options:

| Mode | What it does |
| --- | --- |
| `incremental` | Runs the sync normally, preserving the cache. Only new or changed records are detected. |
| `full_refresh` | Resets `lastSyncDate` and re-fetches all data, but preserves the cache. Nango can still detect which records are new vs. updated. |
| `full_refresh_and_clear_cache` | Resets `lastSyncDate` **and** deletes all cached records. The sync starts completely fresh. |

**Use `incremental` or `full_refresh`** in most cases — the cache is preserved so Nango can accurately compute the delta between what it had before and what is coming in.

**Use `full_refresh_and_clear_cache`** when you need to start from scratch, for example after a breaking change to your data model. Be aware of the implications:
...
- Any cursors you previously persisted become invalid. You must reset your stored cursors when you clear the cache.
