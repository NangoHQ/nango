...
You can implement two-way syncs by combining syncs with [Actions](/guides/use-cases/actions).
...
- Syncs run in the context of each [Connection](/guides/use-cases/api-auth#what-is-a-connection%3F) (API key or access token of your user)
...
Syncs are defined on the integration (e.g., GitHub, Google Drive) by implementing a [Function](/guides/platform/functions) with the sync's logic. Nango then executes the sync for each Connection of this integration, letting you customize logic and frequency per customer if needed.
...
Follow the [deletion detection guide](/implementation-guides/syncs/deletion-detection) for implementation details.

### Real-time syncs with webhooks

Nango supports real-time syncs using [webhooks](/guides/use-cases/webhooks). You can rely entirely on webhooks or combine them with polling to ensure you never miss data.

Follow the [real-time syncs guide](/implementation-guides/syncs/realtime-syncs) to add real-time support.
