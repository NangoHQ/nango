...
<Warning>
`deleteRecordsFromPreviousExecutions()` is incompatible with checkpoints because it requires comparing the full dataset between consecutive runs. Use `trackDeletesStart`/`trackDeletesEnd` instead — see [Deletion detection](/implementation-guides/use-cases/syncs/deletion-detection).
</Warning>
