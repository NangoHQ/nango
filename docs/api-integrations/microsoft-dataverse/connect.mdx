...
Sign in to your Dataverse environment, then copy its domain from the browser's address bar — everything after `https://` and before the first `/` (for example, `contoso.api.crm.dynamics.com`).
