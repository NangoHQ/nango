...
<Steps>
  ...
  <Step title="Verify your app">
   Most Gmail scopes are marked "sensitive" or "restricted" by Google. You need to pass a Google review to go live.
...
When configuring your OAuth app, you'll need to select the appropriate scopes for your use case. Select only the permissions your app needs to function.

For the full list of available Gmail scopes and their descriptions, see [Google's OAuth 2.0 Scopes documentation](https://developers.google.com/identity/protocols/oauth2/scopes#gmail).

For more details on Google's OAuth implementation, see the [Gmail API documentation](https://developers.google.com/gmail/api/reference/rest).
