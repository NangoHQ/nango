...
3. **Authentication** (required): Enable authentication and configure a service account. Set the audience to your Nango webhook URL from the previous step. Google then signs every push with an OIDC JWT that Nango verifies.
4. **Save the changes**

<Warning>
Nango rejects a push that arrives with no `Authorization` header, and the notification is not forwarded to your app. A subscription created without authentication will silently deliver nothing.
</Warning>
