...
Optionally, you can also set a **Scope** to request write access. Simpplr issues read-only tokens by default.
...
Simpplr accepts three access levels and issues `read` tokens when no scope is requested:

| Scope | Access |
|-------|--------|
| `read` | GET calls, and the search endpoints that take a POST body. This is Simpplr's default. |
| `write` | POST, PUT and DELETE calls. |
| `everything` | Both read and write. |

Leave the **Scope** field empty for read-only access. Pick `write` or `everything` only if your integration modifies Simpplr content.
...
5. Optionally select a **Scope** from Step 3. Leave it empty for read-only access.
