...
Simpplr issues read-only tokens by default. To request write access, set `oauth_scopes` on the connection to `write` or `everything`; Simpplr's client-credentials grant accepts `read`, `write` and `everything`.
