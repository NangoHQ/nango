...
2. Nango verifies the delivery's signature against the **Webhook secret** on the integration, then matches the event's `payload.account_id` against `connection_config.accountId` on your connections — the same **Account ID** you entered when connecting (see [how do I link my account?](/api-integrations/zoom-cc/connect#step-1-creating-a-server-to-server-oauth-app)).
...
Nango matches `payload.account_id` against `connection_config.accountId` on your connections — this is the only routing strategy for this provider; a `nangoConnectionId` query param on the webhook URL is not used.
