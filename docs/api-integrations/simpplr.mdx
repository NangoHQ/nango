...
- The access token is a JWT that lasts 24 hours, but the token response carries only `access_token` with no `expires_in`, so Nango refreshes on its default client-credentials schedule, well inside that window.
