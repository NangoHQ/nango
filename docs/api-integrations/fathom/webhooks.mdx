...
2. Nango verifies the delivery's signature (if you've set a **Webhook secret** on the integration — required when routing by `nangoConnectionId`), then routes the event to a connection.
...
2. Enter the secret Fathom returned in the previous step into the **Webhook Secret** field.

<Note>
If no **Webhook secret** is set, Nango accepts email-routed deliveries without verifying their signature. Deliveries using `nangoConnectionId` always require a **Webhook secret** — see [Connection matching](#connection-matching).
</Note>
...
- **By `nangoConnectionId`** — append `?nangoConnectionId=<CONNECTION-ID>` to the destination URL when creating the webhook. Every delivery for that webhook routes to that one connection regardless of who recorded it. Use this when a single connection should receive everything for the workspace. Because the connection is chosen from a value in the URL, Nango requires a **Webhook secret** to be set on the integration for this mode — a delivery with `nangoConnectionId` is rejected if no secret is configured or its signature doesn't verify.
