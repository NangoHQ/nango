...
Nango extracts the calendar owner from the `X-Goog-Resource-URI` header (e.g. `.../calendars/user@example.com/events...`) to match the webhook to a connection. For new connections, Nango persists the user's email address automatically at connection time — no manual setup required.
...
1. `connection_config.emailAddress` — set by Nango for connections that persist it (e.g. via post-connection script).
2. `metadata.emailAddress` — legacy fallback.
3. `metadata.email` — fallback.

For **new connections**, use a [post-connection-creation script](/implementation-guides/use-cases/implement-event-handler) to fetch the user’s email (e.g. from the Calendar API or OAuth profile) and store it in metadata or connection config so webhooks resolve to the right connection.

For **existing connections** that don’t have this set, you can set metadata via the API:
...
<Warning>
If Nango cannot match the webhook to a connection (no matching email in connection config or metadata), the webhook is still received but the forwarded payload will not include a `connectionId`.
</Warning>
