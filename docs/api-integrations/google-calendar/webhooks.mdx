...
**Duplicates:** If more than one connection lists the **same** URI in `googleCalendarWatchResourceUris`, **every** matching connection is included. Nango forwards the webhook **once per matching connection**, with appropriate connection id.
...
<Warning>
If Nango cannot match the incoming webhook to a connection, the webhook will still be received but won't include a `connectionId` in the forwarded payload.
</Warning>
