...
Nango matches `payload.account_id` against `connection_config.accountId` on your connections (populated in [step 4](#4-nango-captures-your-zoom-account-id)) — this is the only routing strategy for this provider; a `nangoConnectionId` query param on the webhook URL is not used.
