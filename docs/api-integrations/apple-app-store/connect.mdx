...
When setting **Scope**, use the format `<HTTP method> <path>`, matching the request you want to allow, e.g. `GET /v1/apps`. See Apple's docs on [generating tokens for API requests](https://developer.apple.com/documentation/appstoreconnectapi/generating_tokens_for_api_requests#4586138) for the full scope syntax, including how to restrict to a specific resource instance with query parameters.
...
4. Optionally, set **Scope** to restrict the token to a single API request. Leave it blank to allow the full set of requests your key has access to.
