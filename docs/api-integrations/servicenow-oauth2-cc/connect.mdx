...
**Error: "integration user is not configured"**
- Ensure the OAuth Application has an **OAuth Application User** set in Step 4.

**Error: "access_denied"**
- Verify the system property `glide.oauth.inbound.client.credential.grant_type.enabled` is set to `true`.
- Check that the OAuth Application User has the required roles (`rest_service`, `itil`).
