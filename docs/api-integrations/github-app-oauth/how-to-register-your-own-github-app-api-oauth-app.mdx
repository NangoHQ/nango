...
-   Webhook payloads are signed with the webhook secret you configured on your GitHub App. The secret is required. Nango verifies the signature on every payload and rejects anything that arrives unsigned. If your app was set up without it, see [Add the webhook secret to an existing GitHub App](/api-integrations/github-app/how-to-register-your-own-github-app-api-app#add-the-webhook-secret-to-an-existing-github-app).
