...
Nango sends these values to `POST https://<server>/ns-api/v2/tokens` with `grant_type=password`, stores the returned access and refresh tokens, and refreshes the access token with `grant_type=refresh_token` before it expires. Your password is only used to obtain the initial token.
...
2. Store the secret securely; it is required both for the initial token request and for every refresh.
