...
Connect to Tripletex with Nango and see data flow in 2 minutes. Tripletex is not OAuth: you supply a consumer token and an employee token, and Nango mints and refreshes a short-lived session token used as the HTTP Basic password on every request.
