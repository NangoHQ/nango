...
You can obtain this from the Salesforce dashboard:
- Log in to Salesforce and copy the domain from your browser’s address bar after login (ensure you only take the hostname, without `https://` or any paths like `/lightning/...`).
- Alternatively, go to **Setup → Company Information** and look for the **Instance** or **My Domain** details.

**Important:**  
Make sure you use the instance hostname (e.g., `acme.my.salesforce.com`) and not the full UI URL (such as `https://acme.lightning.force.com/...`), as this can cause authentication issues.

For more details, refer to Salesforce documentation on instance URLs and domains.
