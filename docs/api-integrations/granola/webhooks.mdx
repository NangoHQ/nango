...
<Tip>
Register a separate Granola webhook endpoint per Nango connection, each pointing at your Nango webhook URL with `?nangoConnectionId=<CONNECTION-ID>` appended. A delivery with no `nangoConnectionId` (or an unknown one) is rejected.
</Tip>
...
- **Multiple connections**: each one has its own webhook endpoint and secret, so set each connection's `signing_secret` as `webhookSecret` in that connection's metadata instead, using [Set connection metadata](/reference/backend/http-api/connections/set-metadata). Nango checks the integration-level **Webhook secret** first and falls back to the connection's metadata when that's not set.
...
<Note>
If neither a **Webhook secret** nor a connection-level `webhookSecret` is set, Nango accepts the delivery without verifying its signature.
</Note>
