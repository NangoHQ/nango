...
<Warning>
If Nango cannot match the incoming webhook to a connection (e.g. the workspace was connected under a different integration key), the webhook will still be forwarded but won't include a `connectionId` in the payload.
</Warning>
