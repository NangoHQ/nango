...
<Steps>
  ...
  <Step title="Enable token rotation">
    Nango supports automatic token refresh for Slack. To use it, enable token rotation in your app manifest:

    1. In the left sidebar, go to **App Manifest**.
    2. Set `token_rotation_enabled` to `true` under the top-level `settings` section:
    ...
    When enabled, Slack issues short-lived access tokens alongside refresh tokens. Nango automatically refreshes both the bot token and the user token before they expire — no action required on your end.

    If token rotation is disabled, access tokens do not expire and Nango will not attempt to refresh them.
  </Step>
