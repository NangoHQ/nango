...
2. **Stripe Context** (Optional) - Only required if you're using an organization API key. The account ID or account path to identify which account the API request should affect.
...
6. In the **Name** field, type a name for your key (e.g., "Nango Integration"). In the **URL** field, enter `https://nango.dev`. Then do one of the following:
   - Click **Create restricted key** to finalize and create an API key that grants access to all permissions, or
   - Select **Customize permissions for this key**. Click **Continue**.
   - Define permissions for your restricted API key and click **Create key**.
