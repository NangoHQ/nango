...
- Tracify Analytics authenticates requests with the `tracify-token` header. Nango sends the configured API token as that header.
...
- This provider is scoped to Tracify Analytics. Its pre-built templates call only read-only retrieval endpoints.
