...
### 2. Set the webhook GUID on the connection

Each Autotask webhook has a unique `Guid`. Nango uses this to route webhooks to the correct connection. Set the `webhookGuid` in the connection's `connectionConfig`:
...
You will get the `Guid` after creating the webhook in Autotask (step 3).

### 3. Create the webhook in Autotask

Copy the webhook URL from your Autotask integration page in the Nango dashboard.

Then create a webhook via the Autotask REST API. Replace the placeholders:
...
Should show `isActive: true` and `isReady: true`. Use the `webhookGUID` value to update the connection config (step 2).
