...
- An Omni account. Creating an Organization API key requires Organization Admin permissions; creating a PAT requires Restricted Querier permissions or higher.
...
Neither token type expires automatically, but both can be disabled, rotated, or revoked from **Settings > API access** (or **Profile > Manage account** for PATs) at any time.
</Note>
