...
Set this value from your trusted backend setup, never by copying an unverified webhook's `Metadata.key_url`. Use an origin only: no path, query, fragment, or credentials. A non-default HTTPS port must match exactly. Existing ConnectWise cloud hosts require no additional metadata.

For a custom origin, Nango first finds connections with the matching Product Instance ID, then accepts the signing-key URL only for connections that explicitly trust that origin. It still verifies the signature before dispatching or forwarding. Redirects and private, loopback, and link-local network targets are blocked, including during DNS lookup. The signing-key endpoint must be publicly reachable over HTTPS.
