...
Custom-origin events reach only connections with a matching Product Instance ID and configured origin, after signature verification. Their key requests block redirects and private network addresses, including during DNS lookup, and require a publicly reachable HTTPS endpoint.
