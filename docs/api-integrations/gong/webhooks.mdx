...
When a public key is configured, Nango also checks that the JWT's `webhook_url` claim exactly matches the destination URL for that connection, including the `nangoConnectionId` query param. This stops a validly-signed delivery for one connection from being replayed against a different connection's route — but it also means the URL you paste into Gong's automation rule must match the one Nango expects byte-for-byte.
</Tip>
