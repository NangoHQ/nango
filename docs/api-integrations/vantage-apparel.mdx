...
```typescript
import type { NangoAction, GetAccountsByEmailInput, GetAccountsByEmailOutput } from '../../models';

function escapeXml(value: string): string {
    return value.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&apos;');
}

export default async function runAction(nango: NangoAction, input: GetAccountsByEmailInput): Promise<GetAccountsByEmailOutput> {
