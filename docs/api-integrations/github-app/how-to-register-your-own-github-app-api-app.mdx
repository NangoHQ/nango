...
<Steps>
    ...
    <Step id="configure-setup-url" title="Configure setup URL">
        ...
        If you left user authorization during installation unselected, under **Setup URL** enter `https://api.nango.dev/app-auth/connect`. This value is the same for every integration in an environment, so you can fill it in now. If you use a custom callback URL, take that URL and replace `oauth/callback` with `app-auth/connect`. Nango also shows the exact value on the integration page once you create it.

        If you selected it, GitHub disables the **Setup URL** field. The connection completes through the callback URL and the installation webhook instead, so there is nothing to enter. Keep the callback URL you set earlier and follow [GitHub App OAuth setup](/api-integrations/github-app-oauth/how-to-register-your-own-github-app-api-oauth-app) for the rest.
        ...
        <Note>The Setup URL is not to be confused with the Callback URL, which can stay empty.</Note>
    </Step>
    ...
    <Step id="create-nango-integration" title="Set up the integration in Nango">
        Create the GitHub App integration in Nango and fill in the **App ID**, **App Private Key** and **App Public Link** you just collected.
