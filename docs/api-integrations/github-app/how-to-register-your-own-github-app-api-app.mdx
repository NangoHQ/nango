...
<Steps>
    ...
    <Step id="configure-setup-url" title="Configure setup URL">
        1. Under **Setup URL**, enter `https://api.nango.dev/app-auth/connect`. This value is the same for every integration in an environment, so you can fill it in now. If you use a custom callback URL, take that URL and replace `oauth/callback` with `app-auth/connect`. Nango also shows the exact value on the integration page once you create it.
        2. Select **Redirect on update** under **Post installation**.
        3. If you selected **Request user authorization (OAuth) during installation** in the previous step, you won't be able to enter a URL here.

        <Warning>
        The setup URL is what completes the connection. GitHub sends the installation ID there after an install, and without it the app installs on GitHub but no connection appears in Nango.
        </Warning>
