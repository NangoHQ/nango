...
Use a [post-connection-creation script](/implementation-guides/platform/functions/event-functions) to automatically register the webhook with Streak whenever a new connection is created. The script generates a secret token, registers the webhook with Streak, and stores the token in the connection config for routing:
