...
export default createOnEvent({
    ...
    exec: async (nango) => {
        // nangoConnectionId lets Nango check the token against this one connection
        const webhookUrl = `${await nango.getWebhookURL()}?nangoConnectionId=${encodeURIComponent(nango.connectionId)}`;
...
<Note>
If the `X-Streak-Webhook-Token` header is missing, or does not match the connection in `nangoConnectionId`, Nango rejects the request. Without `nangoConnectionId`, a token that matches no connection is dropped.
</Note>
