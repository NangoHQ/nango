...
To change the configuration later, send the fields you want to update with [Update an integration](/reference/backend/http-api/integration/update) (`PATCH /integrations/{uniqueKey}`) or `nango.updateIntegration()`. The API key itself is provided per connection, not on the integration. See [Connect to Private API (Generic)](/integrations/all/private-api-generic/connect).
