...
import TemplateFunctions from "/snippets/generated/microsoft-tenant-specific/TemplateFunctions.mdx"
...
<Tip>You _can_ use this provider for general Microsoft Graph access (and the major Microsoft SaaS services like Teams, OneDrive, other Office 365 services, etc.) by setting the `tenant` to `common` but we recommend using the [Microsoft Teams provider](/api-integrations/microsoft-teams) instead which includes sync capability.</Tip>

<Note>Contribute API gotchas by [editing this page](https://github.com/nangohq/nango/tree/master/docs/integrations/all/microsoft-tenant-specific.mdx)</Note>