  ...
  <Tab title="🚀 Quickstart">
    <Steps>
      ...
      <Step title="Call the ContactOut API">
        Let's make your first request to the ContactOut API (organization stats). Replace the placeholders below with your [Environment API key](/reference/backend/http-api/api-keys), [integration ID](https://app.nango.dev/dev/integrations), and [connection ID](https://app.nango.dev/dev/connections):
        <Tabs>
            <Tab title="cURL">
                ```bash
    ...
    <Tip>
    Next step: [Embed the auth flow](/guides/primitives/auth) in your app so your users can add their ContactOut API keys themselves.
    </Tip>
  </Tab>
