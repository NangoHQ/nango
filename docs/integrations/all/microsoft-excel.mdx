  ...
  <Tab title="🚀 Quickstart">
    <Steps>
      ...
      <Step title="Call the Microsoft Excel API">
        ...
        <Note>
         The `{fileId}` in the URL is the unique identifier for the Excel file in OneDrive/SharePoint. You can obtain this ID by first making a Nango [proxy](https://docs.nango.dev/reference/api/proxy/get) call to `/v1.0/me/drive/root/children` to list files and retrieve their IDs. Then, use the ID of the desired workbook to access its worksheets.
         </Note>
