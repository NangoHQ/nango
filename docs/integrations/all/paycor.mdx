...
import TemplateFunctions from "/snippets/generated/paycor/TemplateFunctions.mdx"
...
- **Subscription Key Required**: You need to add your **API Subscription Key** (found in the Security Connections tab) as a connection configuration parameter in Nango for all API calls to work properly. See [creating a connect session documentation](/reference/api/connect/sessions/create) for details on how to add `subscriptionKey` to your `connection_config` when creating a Nango session.
