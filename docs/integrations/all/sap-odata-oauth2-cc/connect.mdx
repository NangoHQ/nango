...
3. **API Server** - The API base URL of the SAP S/4HANA cloud API service you want to connect to.
...
16. Create a new integration user to whom you will assign OAuth scopes. In the **Address** tab, enter the **Last name**. In the **Logon Data** tab:
   - Set a **New Password**. We recommend using the **Generate** option to create a complex string for enhanced security. This password will be used as the **Client Secret**.
...
20. Select the **Grant Type Client Credentials Active** checkbox and leave the other settings unchanged. 
...
2. You can locate this in your SAP BTP Cockpit by checking the URL in your browser's address bar. The subdomain is the value between `https://` and `.hana`.
...
4. Format: `<hostname>` (without `https://` protocol or path segments).
