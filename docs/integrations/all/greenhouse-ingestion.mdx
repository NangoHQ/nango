  ...
  <Tab title="🚀 Quickstart">
    <Steps>
      <Step title="Create an integration">
        In Nango ([free signup](https://app.nango.dev)), go to [Integrations](https://app.nango.dev/dev/integrations) -> _Configure New Integration_ -> _Greenhouse (Ingestion API)_. Nango doesn't provide a test OAuth app for Greenhouse (Ingestion API) yet, so you'll need to set up your own by following these [instructions](#oauth-app-setup). After that, make sure to add the OAuth Client ID, Client Secret, and scopes in the integration settings in Nango.
      </Step>
