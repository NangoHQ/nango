...
<Steps>
  ...
  <Step title="Create a Google Drive integration">
    ...
    - Copy the "Project Number" which can be found in the Google Cloud console under "IAM" > "Settings" > "Project Number"
  ...
  <Step title="Prepare your env">
    ...
    cp .env.example .env
    ```
    - Paste the value you copied for your "Project Number" from the Google Console into the .env file as `GOOGLE_PROJECT_NUMBER=your_project_number_here`:
    ```
    cd front-end
    cp .env.example .env
    ```
  ...
  <Step title="Bonus: Deploy custom functions">
    ...
    - Optionally customize the integration using [this guide](/guides/platform/functions).
