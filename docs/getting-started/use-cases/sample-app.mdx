...
<Steps>
  ...
  <Step id="prepare-environment" title="Prepare your env">
    ...
    npm i
    ```
    - Copy your Nango Environment API key, found in [Environment Settings > API Keys](https://app.nango.dev/dev/environment-settings#api-keys?source=sample-app).
    - Create a file to store environment variables and fill in the Nango Environment API key:
    ```sh
    cp .env.example .env
    ```
  ...
  <Step id="deploy-custom-functions" title="Bonus: Deploy custom functions">
    ...
    nango init
    ```
    - Add your Nango Environment API key in `NANGO_SECRET_KEY_PROD` in `./nango-integrations/.env`.
