...
<Steps>
  <Step title="Create an integration">
    [Sign up](https://app.nango.dev/signup) for free, then go to the Integrations tab > Set up new integration > pick an API. This quickstart uses GitHub (User OAuth).
    ...
    <Accordion title="For agents">
      ...
      Use the API key as `Authorization: Bearer <NANGO-API-KEY>`. The default API key created on signup has full access and is the simplest option. If the user creates a scoped API key instead, this quickstart needs `environment:integrations:write`, `environment:connect_sessions:write`, and `environment:actions:execute`.

      If the user wants another API, list providers first and use the matching `name` as the provider ID:
      ...
      Then create the integration with the API. For GitHub, the provider ID is `github`:
      ...
      Use another provider ID and integration ID if you are testing with a different API. For all fields and response shapes, see the [List all providers API](/reference/backend/http-api/providers/list) and [Create an integration API](/reference/backend/http-api/integration/create).
    </Accordion>
  </Step>

  <Step title="Authorize the API">
    Go to the Connections tab > Add Test Connection > pick your integration, and complete the auth flow.

    Copy the integration ID, connection ID, and your Nango API key from the Environment settings tab > API Keys. You will use them in the next step.
  ...
  <Step title="Inspect the run">
    Open the [Logs](https://app.nango.dev/dev/logs) tab to inspect the function execution, provider request, response, and any errors.
