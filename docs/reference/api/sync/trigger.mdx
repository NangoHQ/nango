...
Use `reset` to clear the checkpoint and re-fetch all data, or `reset` with `emptyCache` to start completely fresh. See the [checkpoints guide](/guides/functions/checkpoints#re-syncing-resetting-checkpoints-and-the-cache) for details.