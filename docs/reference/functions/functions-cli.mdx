...
Get your `prod` and `dev` Environment API keys from [Environment Settings > API Keys](https://app.nango.dev/environment-settings#api-keys) (toggle between the `prod` and `dev` environment in the left nav bar).

<Info>
For self-hosting, set the `NANGO_HOSTPORT` env variable to `http://localhost:3003` (for local development) or your instance's URL.

If the public API is behind mutual TLS, set `NANGO_CLI_TLS_CERT` to a PEM file path. A file that contains both the certificate and private key is enough. Use `NANGO_CLI_TLS_KEY` and `NANGO_CLI_TLS_CA` when the key or CA is in a separate file.
</Info>
