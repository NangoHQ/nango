...
<ResponseField name="checkpoint" type="ZodSchema">
  ...
  Use checkpoints to save progress and avoid re-fetching all data on every run. See the [checkpoints guide](/implementation-guides/platform/functions/checkpoints) for details.
...
Sync functions persist data updates to the Nango cache, which your app later fetches. See the [sync functions section](/implementation-guides/platform/functions/sync-functions).
...
To use checkpoints, define a `checkpoint` schema in your sync configuration (see [Configuration](#createsync)). For a complete guide on implementing incremental syncs with checkpoints, see the [checkpoints guide](/implementation-guides/platform/functions/checkpoints). If you are migrating from `nango.lastSyncDate`, see the [migration guide](/implementation-guides/platform/migrations/migrate-to-checkpoints).
...
To implement deletion detection in your syncs, [follow this guide](/implementation-guides/platform/functions/deletion-detection).
...
For more details on deletion detection strategies, see the [detecting deletes guide](/implementation-guides/platform/functions/deletion-detection).
