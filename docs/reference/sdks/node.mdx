...
If you do not want to deal with collecting & injecting credentials in requests for multiple authentication types, use the Proxy ([step-by-step guide](/guides/platform/proxy/implement-requests-proxy)).
</Info>
...
<Expandable>
    ...
    <ResponseField name="opts" type="object">
        Options for triggering the sync. See the [checkpoints guide](/guides/functions/checkpoints#re-syncing-resetting-checkpoints-and-the-cache) for details on `reset` and `emptyCache`.
        <Expandable>
...
The `checkpoint` field contains the last saved checkpoint for the sync. Learn more about [checkpoints](/guides/functions/checkpoints).
</Expandable>
...
Makes an HTTP request using the [proxy](/guides/platform/proxy/implement-requests-proxy):
