...
<Expandable>
    ...
    <ResponseField name="opts" type="object">
        Options for triggering the sync. See the [checkpoints guide](/implementation-guides/platform/functions/checkpoints#re-syncing-resetting-checkpoints-and-the-cache) for details on `reset` and `emptyCache`.
        <Expandable>
...
The `checkpoint` field contains the last saved checkpoint for the sync. Learn more about [checkpoints](/implementation-guides/platform/functions/checkpoints).
</Expandable>
...
Makes an HTTP request using the [proxy](/implementation-guides/platform/proxy/implement-requests-proxy):
