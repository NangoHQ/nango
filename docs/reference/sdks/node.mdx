...
If you do not want to deal with collecting & injecting credentials in requests for multiple authentication types, use the Proxy ([step-by-step guide](/implementation-guides/platform/proxy/implement-requests-proxy)).
</Info>
...
<Expandable>
    ...
    <ResponseField name="opts" type="object">
        Options for triggering the sync. See the [checkpoints guide](/implementation-guides/use-cases/syncs/checkpoints#re-syncing-resetting-checkpoints-and-the-cache) for details on `reset` and `emptyCache`.
        <Expandable>
...
The `checkpoint` field contains the last saved checkpoint for the sync. Learn more about [checkpoints](/implementation-guides/use-cases/syncs/checkpoints).
</Expandable>
...
<Expandable>
    ...
    <ResponseField name="frequency" type="string" required>
        The frequency you want to set (ex: 'every hour'). Set to `null` to revert to the default frequency. Uses the https://github.com/vercel/ms notations. See [minimum sync frequency](/guides/platform/limits#minimum-sync-frequency) for details.
    </ResponseField>
</Expandable>
...
Makes an HTTP request using the [proxy](/guides/primitives/proxy):
...
<Expandable>
  ...
  <ResponseField name="integrations_config_defaults" type="object">
    Default configuration for specific integrations. Each entry can include `connection_config` (params rendered as additional form inputs) and `credentials` (values that pre-populate and hide credential fields such as IAM Role ARN or the AWS SigV4 `external_id`).
  </ResponseField>
</Expandable>
...
<Expandable>
  ...
  <ResponseField name="integrations_config_defaults" type="object">
    Default configuration for specific integrations. Include `connection_config` entries for parameter defaults and `credentials` entries to hide/pre-populate credential fields (for example `role_arn`, `region`, or `external_id`).
  </ResponseField>
</Expandable>
