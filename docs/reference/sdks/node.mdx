...
await nango.listConnections({
    ...
    // only supports email | displayName
...
If you do not want to deal with collecting & injecting credentials in requests for multiple authentication types, use the Proxy ([step-by-step guide](/implementation-guides/requests-proxy/implement-requests-proxy)).
</Info>
...
Deletes records for a given model and connection up to a specified cursor. Supports both soft deletion (marks records as deleted) and hard deletion (permanently removes records).

```ts
const result = await nango.deleteRecords({
    providerConfigKey: '<INTEGRATION-ID>',
    connectionId: '<CONNECTION-ID>',
    model: '<MODEL-NAME>',
    mode: 'soft',
...
console.log(`Deleted ${result.count} records. More records available: ${result.has_more}`);
```
...
<Expandable>
  <ResponseField name="config" type="object" required>
    <Expandable title="config" defaultOpen>
      ...
      <ResponseField name="model" type="string" required>
        The name of the model from which to delete records.
      </ResponseField>
      ...
      <ResponseField name="mode" type="'soft' | 'hard'" required>
        The deletion mode:
        - `soft`: Marks records as deleted.
        - `hard`: Permanently removes records from the database
      </ResponseField>

      <ResponseField name="untilCursor" type="string" required>
        The cursor up to which records should be deleted. Only records with a cursor less than or equal to this value will be deleted.
      </ResponseField>

      <ResponseField name="limit" type="number">
        The maximum number of records to delete in this operation.
      </ResponseField>
    </Expandable>
  </ResponseField>
</Expandable>
...
<ResponseField name="count" type="number">
  The number of records that were deleted in this operation.
</ResponseField>

<ResponseField name="has_more" type="boolean">
  Indicates whether there are more records available for deletion that match the criteria. If `true`, you must call the method again to continue deleting records.
</ResponseField>
</Expandable>
...
Makes an HTTP request using the [proxy](/guides/use-cases/proxy):
...
Create a connect session for a given end user. The token is short-lived and lasts for 30 minutes.
