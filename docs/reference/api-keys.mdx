...
<Tip>
  For local development with `nango dryrun`, use a key with full access on the dev environment. For CI/CD pipelines deploying to production, create a dedicated key with only the `environment:deploy` scope.
</Tip>
