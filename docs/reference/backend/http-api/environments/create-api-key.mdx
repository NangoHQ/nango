...
Creates an Environment API key in the given environment. Keys created by this endpoint always have full environment access (`environment:*`). Custom scopes are not supported here — create scoped keys from **Environment Settings > API Keys**.
