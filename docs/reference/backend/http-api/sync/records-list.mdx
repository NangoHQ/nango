...
<Warning>
A response may contain fewer records than `limit`. To protect server memory, Nango caps each response by total payload size, so large records can cause the page to be truncated before `limit` is reached. This does not mean there are no more records — always check `next_cursor` and keep paginating until it is `null`.
</Warning>
