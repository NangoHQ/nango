...
<Tabs>
  <Tab title="Node SDK">
    ...
    const nango = new Nango({ secretKey: '<YOUR-ENVIRONMENT-API-KEY>' });
    ```
  </Tab>
  <Tab title="cURL">
    ```bash
...
They currently receive full account access (`account:*`) and can only be managed by account administrators.
...
Creating a production environment also requires the `account:environments:set_production` scope (included in `account:*`):
...
curl --request POST \
  ...
  --header 'Authorization: Bearer <YOUR-ACCOUNT-API-KEY>' \
  --header 'Content-Type: application/json' \
  --data '{ "name": "prod", "is_production": true }'
```

### Account API key scopes

Account API keys created in the dashboard currently receive the `account:*` scope, which grants access to every account-level API. Granular scopes used by environment-management endpoints:

| Scope | Description |
|-------|-------------|
| `account:environments:create` | Create non-production environments |
| `account:environments:delete` | Delete environments other than the protected `prod` environment |
| `account:environments:set_production` | Create a production environment; required in addition to `account:environments:create` |
