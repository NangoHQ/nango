...
API keys grant access to a single environment. Use them with environment-level APIs for integrations, connections, functions, records, and proxy requests. Each key can have full access or a custom set of environment scopes.

Manage API keys under **Environment Settings > API Keys**.

### Account API keys

Account API keys grant access to account-level APIs and do not grant access to any environment. They currently have full account access and can only be managed by account administrators.

Manage Account API keys from **Account API keys** in the profile menu.
