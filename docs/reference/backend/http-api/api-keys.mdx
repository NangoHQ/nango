...
They have full account access and can only be managed by account administrators.
...
Account API keys created in the dashboard currently receive the `account:*` scope, which grants access to every account-level API. The following granular scopes define the permissions required by environment-management endpoints:
