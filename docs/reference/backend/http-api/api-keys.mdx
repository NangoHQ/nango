...
For services that compile function source code without deploying it:
...
The **Default - Full access** key that comes with each environment already has all required scopes for compiling, deploying, and dry-running. For production CI/CD pipelines, consider creating a dedicated key with only the `environment:deploy` scope to follow the principle of least privilege.
