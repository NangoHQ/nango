...
- **[Account API keys](/reference/backend/http-api/api-keys#account-api-keys)** — access account-level APIs only, such as [environment management](/reference/backend/http-api/environments/create). Manage them from [Account API keys](https://app.nango.dev/api-keys) in the dashboard. They do not grant direct access to any environment, but can be used to create Environment API keys.
