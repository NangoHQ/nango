...
- **[Account API keys](/reference/backend/http-api/api-keys#account-api-keys)** — access [account-level APIs](/reference/backend/http-api/api-keys#account-level-apis) only. Requests to environment-level APIs with an Account API key return a `403 Forbidden` response. Manage them from [Account API keys](https://app.nango.dev/api-keys) in the dashboard.
