...
| `connections_get`  | Get a connection and its current credential state. Credential access and refresh flags require `read_credentials`, may rotate credential material, and are audited. | `environment:connections:read` or `environment:connections:read_credentials` |
