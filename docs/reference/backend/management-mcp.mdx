...
| `connections_get`  | Get a connection and its current credential state. Explicit credential refresh flags can refresh or rotate credential material.       | `environment:connections:read` or `environment:connections:read_credentials` |
