...
For providers that declare an `integration_config` schema (e.g. `private-api-generic`, `aws-sigv4`), pass `integration_config` to set the provider-specific configuration:
...
<Expandable>
    ...
    <ResponseField name="integration_config" type="Record<string, string>">
        Provider-specific configuration for providers that declare an `integration_config` schema. The keys and required fields depend on the provider, and values are validated against the provider schema.
    </ResponseField>
</Expandable>
...
Update the provider-specific configuration of a `private-api-generic` (or other `integration_config`) integration:

```js
await nango.updateIntegration(
    { uniqueKey: 'acme-internal' },
    {
        integration_config: {
...
<Expandable>
    ...
    <ResponseField name="integration_config" type="Record<string, string>">
        Provider-specific configuration for providers that declare an `integration_config` schema (e.g. `private-api-generic`, `aws-sigv4`). Only the fields you pass are updated. See [Create an integration](#create-an-integration) for the `private-api-generic` fields.
    </ResponseField>
</Expandable>
